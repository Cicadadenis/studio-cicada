...
Синтаксис поддерживается:
    ...
    картинка "url"
    стикер "file_id"
    отправить файл <выражение>   — sendDocument по file_id, URL или пути (см. также документ …)
    переключить переменная: "вариант": ...
    fetch "url" → переменная / fetch_json "url" → переменная
    разобрать_json переменная → переменная
"""
...
@dataclass
class FetchJson:
    """HTTP GET запрос с разбором JSON-ответа"""
    url: str
    variable: str
    headers: dict = field(default_factory=dict)
...
class Parser:
    ...
    def _parse_stmt(self, line: str):
        ...
        if m:
            return FetchJson(url=m.group(1), variable=m.group(2))

        # разобрать_json источник → переменная
        m = re.match(r'^разобрать_json\s+(.+?)\s*(?:→|->)\s*(\w+)$', line)
        if m:
            return ParseJson(source=parse_value(m.group(1).strip()), variable=m.group(2))
