# Migration Architecture: `studio-cicada` → `cicada-tg==0.3.5`

> Scope note: current alignment target is the installed `cicada-tg==0.3.5` package from `/usr/local/lib/python3.12/dist-packages`.
...
### B. Target model (`cicada-tg==0.3.5` as source of truth)
...
> Function names below are integration targets; confirm exact names/signatures in `cicada-tg==0.3.5` docs.
...
[React UI / DSL Editor / Visual Builder]
     ...
     [Studio Backend API Gateway]
                ...
                | 4) POST /media/*
                v
      [cicada-tg Runtime Service 0.3.5]
