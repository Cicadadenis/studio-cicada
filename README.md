...
sudo CICADA_TG_PIN=0.3.4 bash bootstrap.sh
```

Скрипт:

- ставит системные зависимости (**Node.js 20**, **PM2**, **PostgreSQL**, **Nginx** — кроме Termux), ставит **`cicada-tg`** с PyPI (по умолчанию **0.3.4**); при наличии `cicada` в `PATH` пропишет актуальный **`CICADA_BIN`** в `.env`;
