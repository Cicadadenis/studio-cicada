    ...
    <div style={{ overflowY: 'auto', flex: 1 }}>
      {GROUPS_ORDER.map(gname => {
        ...
        return (
          <div key={gname}>
            <div style={{
              padding: '8px 12px 3px', fontSize: 9, color: 'var(--text3)',
...
// ─── DSL VALIDATOR ────────────────────────────────────────────────────────
// Логика проверки основана на поведении executor.py (dispatch-таблица ядра).
//
// Что считается "выводом" (согласно _dispatch в Executor.__init__):
//   Reply, Buttons, InlineButton, Photo, PhotoVar, Sticker,
//   SendDocument, SendAudio, SendVideo, SendVoice,
//   SendLocation, SendContact, SendPoll, SendInvoice, SendGame,
//   SendMarkdown, ForwardPhoto
// Блоки, которые делегируют вывод (не требуют прямого ответа):
//   UseBlock (использовать), StartScenario (перейти), GotoStep, стоп
// Блоки, которые сами задают вопрос пользователю:
//   Ask (спросить) — тоже является выводом (флашит pending_message)

function getLineIndent(rawLine) {
  return (rawLine.replace(/\t/g, '    ').match(/^(\s*)/)?.[1]?.length) ?? 0;
}

function validateDSL(code, stacks) {
  const errors = [];
  const warnings = [];
  ...
  stacks.forEach(stack => {
    ...
    if (!hasOutput && !hasDelegate && !hasCondition) {
      const label = getBlockDef(root.type)?.label || root.type;
  ...
  stacks.forEach(stack => {
    stack.blocks.forEach(b => {
      if (b.type === 'goto' && b.props?.target) {
        const target = b.props.target;
        ...
        if (!isValid) {
          errors.push(`❌ «перейти "${target}"» — сценарий или команда с таким именем не объявлены`);
        }
      }
    });
  });
  ...
  const DSL_KEYWORDS = new Set([
    ...
    'глобально','пауза','печатает','фото','аудио','видео','документ',
...
function OnboardingTour({ steps, stepIndex, onNext, onPrev, onSkip }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 20000, pointerEvents: 'none' }}>
      ...
      {targetRect && (
        <div
          style={{
            ...
            boxShadow: '0 0 0 9999px rgba(2,6,12,0.58), 0 0 26px rgba(255,215,0,0.45)',
      ...
      >
        <div style={{ fontSize: 10, color: 'rgba(255,215,0,0.7)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              onClick={onNext}
              style={{ background: 'linear-gradient(135deg,#ffd700,#f59e0b)', color: '#111', border: 'none', borderRadius: 8, padding: '7px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}
            >
              {isLast ? 'Готово' : 'Далее'}
            </button>
          </div>
...
`
  ...
  const authModalNode = showAuthModal ? (
    <AuthModal
      ...
      onLogin={async (email, password, tgData) => {
        let user;
        if (tgData) {
  ...
  if (!currentUser) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(155deg,#05070c 0%,#080d18 48%,#05070c 100%)', color: '#fff', fontFamily: 'system-ui,sans-serif' }}>
        <style>{`
          @keyframes landingGrid { from{background-position:0 0} to{background-position:0 34px} }
          ...
          .lp-btn-ghost { background:rgba(255,255,255,0.04); color:rgba(255,255,255,0.88); border-radius:8px; border:1px solid rgba(255,255,255,0.14); font-size:13px; font-weight:600; cursor:pointer; transition:all .2s; padding:9px 16px; }
          .lp-btn-ghost:hover { background:rgba(255,255,255,0.09); border-color:rgba(255,255,255,0.3); }
          .lp-btn-gold { background:linear-gradient(135deg,#ffd700,#f59e0b); color:#111; border:none; font-weight:800; cursor:pointer; transition:all .2s; font-family:Syne,system-ui; }
          .lp-btn-gold:hover { filter:brightness(1.08); box-shadow:0 6px 24px rgba(245,158,11,0.4); transform:translateY(-1px); }
          ...
          .lp-nav-link { background:none; border:none; color:rgba(255,255,255,0.6); font-size:13px; cursor:pointer; transition:color .2s; padding:4px 2px; }
        ...
        <div style={{ position:'fixed', inset:0, pointerEvents:'none', zIndex:0 }}>
          <div style={{ position:'absolute', top:'-10%', left:'-5%', width:'55%', height:'55%', background:'radial-gradient(ellipse,rgba(245,158,11,0.13) 0%,transparent 65%)' }} />
          <div style={{ position:'absolute', top:'5%', right:'-10%', width:'50%', height:'50%', background:'radial-gradient(ellipse,rgba(59,130,246,0.12) 0%,transparent 65%)' }} />
          <div style={{ position:'absolute', top:0, left:0, right:0, bottom:0, backgroundImage:'linear-gradient(to bottom,rgba(255,255,255,0.03) 1px,transparent 1px)', backgroundSize:'100% 34px', opacity:.18, animation:'landingGrid 14s linear infinite' }} />
          {/* Stars */}
          {[...Array(28)].map((_,i) => (
          ...
          {/* Floating orbs */}
          <div style={{ position:'absolute', top:'25%', right:'18%', width:320, height:320, borderRadius:'50%', background:'radial-gradient(circle,rgba(124,110,242,0.08) 0%,transparent 65%)', animation:'orbFloat 12s ease-in-out infinite' }} />
          <div style={{ position:'absolute', top:'55%', left:'8%', width:200, height:200, borderRadius:'50%', background:'radial-gradient(circle,rgba(245,158,11,0.06) 0%,transparent 65%)', animation:'orbFloat 9s ease-in-out infinite reverse' }} />
        </div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:2, flex:1, justifyContent:'center' }}>
              {[
                ...
                <button
                  ...
                  style={{ padding:'6px 14px', fontSize:13, fontFamily:'system-ui,sans-serif' }}
        ...
        <div style={{ position:'relative' }}>
          ...
          {/* ── HERO ── */}
          <div style={{ maxWidth:1220, margin:'0 auto', padding: lp ? '24px 16px 32px' : '36px 40px 80px' }}>
            <div style={{ display:'grid', gridTemplateColumns: lp ? '1fr' : '1fr 1.05fr', gap: lp ? 24 : 40, alignItems:'flex-start', width:'100%' }}>
              ...
              <div className="lp-fadeup">
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:14 }}>
                  {[['⚡','Быстрый старт','Первые результаты за 5 минут'],['🧩','Без кода','Визуальный конструктор и готовые блоки'],['🛡','Надёжно','Стабильная работа и 24/7 доступ']].map(([icon,title,text]) => (
                    <div key={title} style={{ flex: lp ? '1 1 calc(50% - 4px)' : '1 1 30%', minWidth: lp ? 0 : 130, border:'1px solid rgba(255,255,255,0.09)', borderRadius:10, padding: lp ? '6px 9px' : '8px 11px', background:'rgba(255,255,255,0.02)' }}>
                      <div style={{ fontSize: lp ? 11 : 12, color:'#fbbf24', marginBottom:2, fontWeight:600 }}>{icon} {title}</div>
                      <div style={{ fontSize: lp ? 10 : 11, color:'rgba(255,255,255,0.55)' }}>{text}</div>
                    </div>
                  ))}
                </div>
                <div style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'center' }}>
                  <button className="lp-btn-gold" onClick={openRegister} style={{ borderRadius:10, padding:'10px 20px', fontSize:14 }}>Начать бесплатно →</button>
                </div>
              ...
              {!lp && (
                <div style={{ position:'relative', animation:'panelFloat 5.8s ease-in-out infinite', paddingTop:28, overflow:'visible' }} className="lp-fadeup">
                  ...
                  <div style={{ position:'relative', zIndex:1, border:'1px solid rgba(255,255,255,0.1)', borderRadius:20, background:'linear-gradient(180deg,rgba(14,18,28,0.98),rgba(8,11,18,0.99))', overflow:'hidden', boxShadow:'0 30px 80px rgba(0,0,0,0.6)' }}>
                    {/* titlebar */}
                  ...
                  {/* glow under */}
                  <div style={{ position:'absolute', bottom:-40, left:'50%', transform:'translateX(-50%)', width:260, height:60, background:'radial-gradient(ellipse,rgba(124,110,242,0.25) 0%,transparent 70%)', pointerEvents:'none' }} />
                </div>
              )}
            </div>
          </div>
          ...
          <>
          {/* ── STATS ── */}
          <div style={{ borderTop:'1px solid rgba(255,255,255,0.07)', borderBottom:'1px solid rgba(255,255,255,0.07)', background:'rgba(255,255,255,0.015)', padding: lp ? '20px 16px' : '24px 40px' }}>
            <div style={{ maxWidth:1220, margin:'0 auto', display:'grid', gridTemplateColumns: lp ? '1fr 1fr' : 'repeat(4,1fr)', gap: lp ? 16 : 0 }}>
              {[['👥','2 000+','ботов создано','rgba(139,92,246,0.15)'],['⏱','5 мин','среднее время запуска','rgba(251,191,36,0.12)'],['📊','24/7','стабильная работа','rgba(249,115,22,0.15)'],['❤','98%','довольных пользователей','rgba(239,68,68,0.15)']].map(([icon,num,label,bg],i) => (
                <div key={num} style={{ display:'flex', alignItems:'center', gap:14, padding: lp ? '12px 14px' : '0 32px', borderRight: lp ? 'none' : (i<3 ? '1px solid rgba(255,255,255,0.07)' : 'none') }}>
                  ...
                  <div>
                    <div style={{ fontFamily:'Syne,system-ui', fontWeight:800, fontSize: lp ? 22 : 28, color:'#fff', lineHeight:1 }}>{num}</div>
                    <div style={{ fontSize: lp ? 11 : 12, color:'rgba(255,255,255,0.55)', marginTop:3 }}>{label}</div>
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        display: 'flex', alignItems: 'center', padding: isMobileView ? '0 12px' : '0 20px', gap: isMobileView ? 8 : 14,
        flexShrink: 0, height: isMobileView ? 52 : 60,
        overflowX: isMobileView ? 'auto' : 'visible',
      }}>
        <div style={{ fontFamily:'Syne, system-ui', fontWeight:800, fontSize:22, color:'var(--text)', flexShrink: 0 }}>
          <span style={{ color:'#ffd700', textShadow: '0 0 10px rgba(255,215,0,0.5)' }}>◈</span>
          <span style={{ background: 'linear-gradient(135deg, #ffd700 0%, #ffaa00 50%, #ffd700 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', marginLeft: 6 }}>Cicada</span>
          {!isMobileView && <span style={{ fontSize:14, color:'#ffd700', marginLeft:8, fontWeight:400, opacity: 0.9 }}>Studio</span>}
        </div>
        ...
        {!isMobileView && <div style={{ width:1, height:24, background:'var(--border)' }} />}
        ...
        {!isMobileView && (
          <>
            <style>{`
              ...
              .tb-btn-ghost:hover { background: rgba(255,255,255,0.08); color: rgba(255,255,255,0.85); border-color: rgba(255,255,255,0.22); }
              .tb-btn-danger { background: rgba(239,68,68,0.08); color: #f87171; border: 1px solid rgba(239,68,68,0.18); }
              .tb-btn-danger:hover { background: rgba(239,68,68,0.16); color: #fca5a5; border-color: rgba(239,68,68,0.4); }
              .tb-btn-green { background: rgba(62,207,142,0.08); color: #3ecf8e; border: 1px solid rgba(62,207,142,0.2); }
              .tb-btn-green:hover { background: rgba(62,207,142,0.16); border-color: #3ecf8e; color: #6edcac; }
              .tb-btn-blue { background: rgba(96,165,250,0.08); color: #60a5fa; border: 1px solid rgba(96,165,250,0.2); }
              .tb-btn-blue:hover { background: rgba(96,165,250,0.16); border-color: #60a5fa; color: #93c5fd; }
              .tb-btn-purple { background: rgba(167,139,250,0.08); color: #a78bfa; border: 1px solid rgba(167,139,250,0.2); }
              .tb-btn-purple:hover { background: rgba(167,139,250,0.16); border-color: #a78bfa; color: #c4b5fd; }
              .tb-btn-run {
                background: linear-gradient(135deg, #3ecf8e 0%, #2db97a 100%);
                color: #fff; border: none; font-weight: 600; font-size: 13px;
                box-shadow: 0 2px 12px rgba(62,207,142,0.35);
              }
              .tb-btn-run:hover { background: linear-gradient(135deg, #4edea0 0%, #3ecf8e 100%); box-shadow: 0 4px 18px rgba(62,207,142,0.5); transform: translateY(-1px); }
              .tb-btn-run:disabled { background: rgba(62,207,142,0.15); color: rgba(62,207,142,0.35); box-shadow: none; cursor: not-allowed; transform: none; }
              .tb-btn-stop {
                background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%);
                color: #fff; border: none; font-weight: 600;
                box-shadow: 0 2px 12px rgba(239,68,68,0.35);
              }
              .tb-btn-stop:hover { background: linear-gradient(135deg, #f87171 0%, #ef4444 100%); box-shadow: 0 4px 18px rgba(239,68,68,0.5); transform: translateY(-1px); }
              .tb-divider { width: 1px; height: 22px; background: rgba(255,255,255,0.08); flex-shrink: 0; }
              .tb-btn-ai {
              ...
              .tb-btn-ai:hover {
                background: linear-gradient(135deg, rgba(251,191,36,0.28) 0%, rgba(251,146,60,0.28) 100%);
                border-color: rgba(251,191,36,0.65); color: #fde68a;
                box-shadow: 0 2px 14px rgba(251,191,36,0.25); transform: translateY(-1px);
              }
            `}</style>
            <div style={{ position: 'relative' }}>
              <button
            ...
            {currentUser && (
              <button
                ...
                title="Сохранить проект в облако"
              >☁</button>
            )}
            <button
            ...
            <button
              ...
              title="Открыть .ccd файл"
              onClick={loadCCD}
            >↑ .ccd</button>
            <button
              className="tb-btn tb-btn-ghost"
              data-tour="bot-preview"
              title="Чат-симулятор без деплоя в Telegram"
              type="button"
            ...
            <button
              className="tb-btn tb-btn-ghost"
              title="Логи процесса cicada с флагом --dev"
              type="button"
          ...
          <>
            ...
            {isMobileView ? (
              ...
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.background = 'linear-gradient(135deg, rgba(255,215,0,0.22), rgba(255,140,0,0.15))'; e.currentTarget.style.borderColor = '#ffd700'; e.currentTarget.style.boxShadow = '0 0 14px rgba(255,215,0,0.25)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'linear-gradient(135deg, rgba(255,215,0,0.12), rgba(255,140,0,0.08))'; e.currentTarget.style.borderColor = 'rgba(255,215,0,0.3)'; e.currentTarget.style.boxShadow = 'none'; }}
              >
                <span style={{ fontSize: 11 }}>★</span>
                <span style={{ fontSize: 12, fontWeight: 700, color: '#ffd700', fontFamily: 'Syne, system-ui', letterSpacing: '0.02em', whiteSpace: 'nowrap' }}>
                  Premium
                </span>
              </button>
            )}
          ...
          <>
            ...
            <button
              ...
              style={{
                ...
                color: '#1a1a1a', padding: isMobileView ? '7px 14px' : '8px 20px', borderRadius: 6,
                fontSize: isMobileView ? 12 : 13,
                fontWeight: 600, border: 'none', cursor: 'pointer',
                boxShadow: '0 0 10px rgba(255,215,0,0.3)', whiteSpace: 'nowrap',
              }}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ fontSize: 18, fontWeight: 700, color: '#ffd700', fontFamily: 'Syne, system-ui' }}>
                📖 Инструкция Cicada Studio
              </div>
              <button
                onClick={() => setShowInstructions(false)}
                style={{
      ...
      {currentUser ? (
        ...
        <div style={{
          background:'var(--bg2)', borderRight: isMobileView ? 'none' : '1px solid var(--border)',
        ...
        data-tour={!isMobileView ? 'sidebar-desktop' : undefined}>
          <div style={{
            padding:'8px 12px 4px', fontSize:9, color:'var(--text3)',
        ...
        <div
          ...
          style={{
            ...
            background: 'radial-gradient(ellipse at 50% 50%, #1a1d24 0%, #0e0f11 100%)',
        ...
        >
          ...
          <svg style={{ position:'absolute', inset:0, width:'100%', height:'100%', pointerEvents:'none', opacity:0.25 }}>
            <defs>
              <pattern id="dots"
                ...
                <circle cx={canvasScale} cy={canvasScale} r={canvasScale} fill="#3a3d48" />
              </pattern>
            </defs>
              ...
              <div style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    color: canUseAiGenerator ? '#000' : 'rgba(255,255,255,0.45)',
                    border: canUseAiGenerator ? 'none' : '1px solid rgba(255,255,255,0.15)',
                    borderRadius: 14,
                    cursor: canUseAiGenerator ? 'pointer' : 'not-allowed',
                    boxShadow: canUseAiGenerator ? '0 6px 24px rgba(255,215,0,0.3)' : 'none',
                    transition: 'all 0.2s',
                    opacity: canUseAiGenerator ? 1 : 0.85,
                  }}
                  onMouseEnter={e => {
                    if (!canUseAiGenerator) return;
                    e.currentTarget.style.transform = 'translateY(-2px)';
                    e.currentTarget.style.boxShadow = '0 10px 32px rgba(255,215,0,0.45)';
                  }}
                  onMouseLeave={e => {
                    if (!canUseAiGenerator) return;
                ...
                <button
                  ...
                  style={{
                    ...
                    color: '#fde68a', border: '1px solid rgba(251,191,36,0.35)', borderRadius: 12,
                    cursor: 'pointer',
                    transition: 'all 0.2s',
                  }}
                ...
                <div style={{
                  ...
                  <span style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.07)', display: 'block', width: 60 }}/>
          ...
          {/* Zoom controls — +/- only on desktop, pinch on mobile */}
          <div style={{
            position: 'absolute', bottom: isMobileView ? 72 : 16, right: 16,
            ...
            {!isMobileView && (
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg2)'; e.currentTarget.style.borderColor = '#ffd700'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'var(--bg3)'; e.currentTarget.style.borderColor = 'var(--border2)'; }}
              >+</button>
            )}
            <button
              onClick={zoomReset}
              title={`Сброс масштаба (${Math.round(canvasScale * 100)}%)`}
              style={{
              ...
              onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg2)'; e.currentTarget.style.color = '#ffd700'; }}
        ...
        <div style={{
          display:'flex', flexDirection:'column',
          borderLeft: isMobileView ? 'none' : '1px solid var(--border)', overflow:'hidden',
          ...(isMobileView ? { gridColumn: '1', position: 'absolute', top: 0, left: 0, right: 0, bottom: 56, zIndex: 10 } : {}),
        }}
        data-tour={!isMobileView ? 'props-panel-desktop' : undefined}>
          {(!isMobileView || mobileTab === 'props') && (
            <>
              <div style={{
                borderBottom:'1px solid var(--border)', padding:'7px 12px',
                fontSize:9, color:'var(--text3)', textTransform:'uppercase', letterSpacing:'.1em',
              }}>Свойства</div>
        ...
        <div style={{
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                borderTop: `2px solid ${mobileTab === tab.key ? '#ffd700' : 'transparent'}`,
                minWidth: 0,
              }}
            >
              <span style={{ fontSize: 16 }}>{tab.icon}</span>
              <span style={{ fontSize: 9, color: mobileTab === tab.key ? '#ffd700' : 'var(--text3)', fontFamily: 'Syne, system-ui', fontWeight: 600, whiteSpace: 'nowrap' }}>{tab.label}</span>
            </button>
          ))}
          ...
          <button
            ...
            style={{
              ...
              background: isBotRunning ? '#ef4444' : _hasToken ? '#3ecf8e' : '#2d3748',
              border: 'none', cursor: (!isBotRunning && !_hasToken) ? 'not-allowed' : 'pointer',
              borderTop: `2px solid ${isBotRunning ? '#ef4444' : _hasToken ? '#3ecf8e' : '#2d3748'}`,
              borderLeft: '1px solid var(--border)',
              flexShrink: 0, position: 'relative', overflow: 'hidden',
              opacity: (!isBotRunning && !_hasToken) ? 0.45 : 1,
              transition: 'all 0.2s',
            }}
          >
        ...
        <ProfileModal
          ...
          onUpdateUser={async (updates) => {
            try {
              ...
              const updated = await updateUser(currentUser.id, serverUpdates);
          ...
          <div style={{ display: 'flex', gap: 6, padding: 10, borderTop: '1px solid rgba(255,255,255,0.07)' }}>
            <button
              ...
              style={{
                ...
                border: '1px solid rgba(255,215,0,0.35)', background: 'rgba(255,215,0,0.08)',
      ...
      {isStartingBot && (
        <div style={{
          ...
          <div style={{
            ...
            padding: '40px 50px',
            textAlign: 'center',
            boxShadow: '0 40px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,215,0,0.1)',
          }}>
            <div style={{
              width: 60, height: 60,
            ...
            <div style={{ fontSize: 18, color: '#ffd700', fontWeight: 600, marginBottom: 8 }}>
              Запуск бота...
            </div>
...
function GoogleLoginButton() {
  return (
    <button
      ...
      style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '13px 14px', borderRadius: 13, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.9)', fontFamily: 'Syne, system-ui', fontWeight: 700, fontSize: 14, cursor: 'pointer', transition: 'all .2s' }}
    ...
    >
      <span style={{ display: 'inline-grid', placeItems: 'center', width: 24, height: 24, borderRadius: '50%', background: '#fff', color: '#111', fontWeight: 900, fontSize: 13, flexShrink: 0 }}>G</span>
      Войти через Google
    </button>
  );
}
...
function LandingInfoModal({ page, onClose, isMobile }) {
  const [docsSection, setDocsSection] = useState(0);
  ...
  const SectionTitle = ({ children, color = '#fbbf24' }) => (
    <div style={{ fontFamily: 'Syne,system-ui', fontWeight: 700, fontSize: 15, color, marginBottom: 10, marginTop: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
      {children}
    </div>
  );
  const Table = ({ rows }) => (
    <div style={{ overflowX: 'auto', marginBottom: 10 }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
        <thead>
          <tr>
            {rows[0].map((h, i) => <th key={i} style={{ textAlign: 'left', padding: '7px 10px', background: 'rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.7)', fontFamily: 'Syne,system-ui', fontWeight: 700, whiteSpace: 'nowrap' }}>{h}</th>)}
          </tr>
        </thead>
        <tbody>
          {rows.slice(1).map((row, ri) => (
            <tr key={ri} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              {row.map((cell, ci) => <td key={ci} style={{ padding: '7px 10px', color: 'rgba(255,255,255,0.75)', verticalAlign: 'top' }}>{cell}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
  ...
  const DOC_SECTIONS = [
    {
      ...
      content: (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <SectionTitle>Что такое Cicada Studio?</SectionTitle>
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', lineHeight: 1.65, margin: 0 }}>
            <strong style={{ color: '#fbbf24' }}>Cicada Studio</strong> — визуальный конструктор Telegram-ботов на основе ReactFlow.
            Боты создаются путём соединения блоков на холсте, после чего конструктор генерирует код на языке Cicada (<Code>.ccd</Code>).
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: 8 }}>
            {[['Левая панель','Палитра блоков — перетащи на холст','#3ecf8e'],['Центральная область','Холст для построения схемы','#60a5fa'],['Правая панель','Свойства блока + сгенерированный код','#a78bfa']].map(([t,d,c]) => (
              <div key={t} style={{ padding: '12px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: `1px solid ${c}30` }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: c, marginBottom: 4, fontFamily: 'Syne,system-ui' }}>{t}</div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.55)', lineHeight: 1.5 }}>{d}</div>
              </div>
            ))}
          </div>
          <SectionTitle>Пример простого бота</SectionTitle>
          <CodeBlock>{`версия "1.0"
бот "YOUR_BOT_TOKEN"
команды:
    "/start" - "🚀 Запуск бота"
    "/help" - "❓ Помощь"
...
`}</CodeBlock>
    ...
    },
    {
      ...
      content: (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {[
            { group: '⚙ Настройки', color: '#94a3b8', rows: [['Блок','Назначение'],['📌 Версия','Указывает версию бота'],['🤖 Бот','Токен Telegram-бота'],['📋 Команды меню','Команды в меню Telegram'],['🌍 Глобальная','Глобальные переменные']] },
            { group: '▶ Основные', color: '#3ecf8e', rows: [['Блок','Назначение'],['▶ Старт','Точка входа при /start'],['✉ Ответ','Отправка текстового сообщения'],['⊞ Кнопки','Клавиатура с кнопками'],['/ Команда','Обработка /команды'],['⊙ Нажатие','Callback от inline-кнопок']] },
            { group: '🧠 Логика', color: '#fb923c', rows: [['Блок','Назначение'],['◇ Условие','If-else ветвление'],['? Спросить','Запрос ввода от пользователя'],['♦ Запомнить','Сохранить в переменную'],['💾 Сохранить','Постоянное хранение'],['↻ Цикл','Повторение блоков'],['⇄ Переключатель','Switch-case по переменной']] },
            { group: '🚀 Действия', color: '#60a5fa', rows: [['Блок','Назначение'],['↗ HTTP-запрос','API запросы к внешним сервисам'],['⏱ Пауза','Задержка перед следующим блоком'],['→ Переход','Переход к сценарию'],['🗄 БД-запрос','SQL запросы'],['🧠 Классификация','AI-определение намерения']] },
            { group: '🖼 Медиа', color: '#34d399', rows: [['Блок','Назначение'],['🖼 Фото','Отправка изображения'],['▷ Видео','Отправка видео'],['♪ Аудио','Отправка аудио'],['📄 Документ','Отправка файла (PDF, DOC...)'],['◉ Стикер','Отправка стикера']] },
            { group: '↺ Сценарии', color: '#a78bfa', rows: [['Блок','Назначение'],['↺ Сценарий','Определение многошагового сценария'],['» Шаг','Шаг внутри сценария'],['▦ Inline-кнопки','Inline клавиатура под сообщением']] },
    ...
    },
    {
      ...
      content: (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <SectionTitle>Правила соединений</SectionTitle>
          {[
            ['От source к target','Поток выполнения идёт слева направо (правый handle → левый handle)'],
            ['Корневые блоки','Старт, Команда, Сценарий, Нажатие — начало цепочки'],
            ['Блоки-контейнеры','Блок, Middleware, Условие, Цикл — могут иметь вложенные блоки'],
            ['Завершающие блоки','Стоп, Переход — обычно не имеют исходящих соединений'],
          ].map(([t, d]) => (
            <div key={t} style={{ display: 'flex', gap: 12, padding: '10px 12px', borderRadius: 9, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#fbbf24', minWidth: 120, flexShrink: 0 }}>{t}</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)', lineHeight: 1.5 }}>{d}</div>
            </div>
          ))}
          <SectionTitle>Примеры схем</SectionTitle>
          {[
            ['Простое приветствие','[start] → [typing] → [message] → [buttons]'],
            ['Обработка команды','[command /help] → [message]'],
            ['Условная логика','[ask] → [condition] → [message "Да"]\n                    ↓\n               [message "Нет"]'],
            ['Использование блока','[block "greeting"] → [message]\n[start] → [use "greeting"] → [message]'],
          ].map(([t, d]) => (
            <div key={t} style={{ marginBottom: 4 }}>
              <div style={{ fontSize: 11, color: '#60a5fa', fontWeight: 700, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.07em' }}>{t}</div>
    ...
    },
    {
      ...
      content: (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <SectionTitle>Встроенные переменные</SectionTitle>
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', lineHeight: 1.6, margin: 0 }}>В текстовых полях блоков можно использовать фигурные скобки для подстановки значений:</p>
          <Table rows={[
            ['Синтаксис','Значение'],
            ['{пользователь.имя}','Имя пользователя'],
            ['{пользователь.id}','ID пользователя Telegram'],
            ['{чат.id}','ID текущего чата'],
            ['{текст}','Текст последнего сообщения'],
            ['{имя_переменной}','Значение запомненной переменной'],
          ]} />
          <SectionTitle>Пример использования</SectionTitle>
          <CodeBlock>{`ответ "Привет, {пользователь.имя}! Вы посещали нас {счётчик} раз."

спросить "Как вас зовут?" → имя
ответ "Рад познакомиться, {имя}!"`}</CodeBlock>
          <SectionTitle>DSL операторы данных</SectionTitle>
          <CodeBlock>{`запомни счёт = 0
получить "посещений" → счётчик
сохранить "посещений" = счётчик + 1`}</CodeBlock>
        </div>
      ),
    },
    {
      ...
      content: (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <SectionTitle>Панель DSL (справа)</SectionTitle>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 8 }}>
            {[
              ['проверить','Валидация схемы — показывает ошибки и предупреждения','#3ecf8e'],
              ['copy','Копировать сгенерированный код в буфер обмена','#60a5fa'],
              ['↓ .ccd','Скачать файл с кодом для запуска через cicada CLI','#fbbf24'],
              ['▶ Запустить','Запуск бота прямо из браузера (нужен токен в блоке Бот)','#a78bfa'],
            ].map(([t, d, c]) => (
              <div key={t} style={{ padding: '11px 13px', borderRadius: 9, background: 'rgba(255,255,255,0.02)', border: `1px solid ${c}30` }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: c, fontFamily: 'Syne,system-ui', marginBottom: 4 }}>{t}</div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)', lineHeight: 1.5 }}>{d}</div>
              </div>
            ))}
          </div>
          <SectionTitle>Что проверяет валидация</SectionTitle>
          {[
            ['✅','Наличие обязательных полей в блоках'],
            ['✅','Корректность числовых значений'],
            ['✅','Подключение блоков (не висят в воздухе)'],
            ['⚠️','Предупреждение если нет блока «Старт»'],
          ].map(([ico, text]) => (
            <div key={text} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', fontSize: 13, color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>
              <span style={{ flexShrink: 0 }}>{ico}</span><span>{text}</span>
            </div>
          ))}
          <SectionTitle>Локальный запуск (CLI)</SectionTitle>
          <CodeBlock>{`# Установка
pip install cicada-tg

# Запуск бота из файла
cicada bot.ccd`}</CodeBlock>
  ...
  const titles = { features: 'Возможности', templates: 'Шаблоны', docs: 'Документация', pricing: 'Тарифы' };
  const title = titles[page] || '';

  return (
    <div
      style={{ position: 'fixed', inset: 0, zIndex: 15000, background: 'rgba(3,5,9,0.8)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isMobile ? 0 : 18 }}
      onClick={onClose}
    >
      <style>{`
        @keyframes lipSlide { from { opacity:0; transform:translateY(14px) scale(0.98) } to { opacity:1; transform:translateY(0) scale(1) } }
      ...
      `}</style>
      <div
        style={{ width: isMobile ? '100%' : 'min(880px,96vw)', height: isMobile ? '100%' : 'min(680px,92vh)', background: '#0e0f12', borderRadius: isMobile ? '20px 20px 0 0' : 18, border: '1px solid rgba(255,255,255,0.09)', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 32px 80px rgba(0,0,0,0.7)', animation: isMobile ? 'lipSlideUp .3s cubic-bezier(0.34,1.1,0.64,1)' : 'lipSlide .26s cubic-bezier(0.34,1.2,0.64,1)' }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div style={{ padding: isMobile ? '14px 16px' : '16px 22px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, background: '#111318' }}>
          <div style={{ fontFamily: 'Syne,system-ui', fontWeight: 800, fontSize: isMobile ? 18 : 22, color: '#fff' }}>{title}</div>
          <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'none', color: 'rgba(255,255,255,0.4)', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .15s' }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.08)'; e.currentTarget.style.color = '#fff'; }} onMouseLeave={e => { e.currentTarget.style.background = 'none'; e.currentTarget.style.color = 'rgba(255,255,255,0.4)'; }}>×</button>
        </div>
        ...
        <div style={{ flex: 1, overflow: 'hidden', display: 'flex' }}>
          ...
          {/* ── FEATURES ── */}
          {page === 'features' && (
            <div className="lip-scroll" style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '16px' : '24px' }}>
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', lineHeight: 1.65, marginBottom: 20, marginTop: 0 }}>Cicada Studio — полноценная платформа для создания Telegram-ботов без написания кода вручную. Всё что нужно — собрать схему блоками.</p>
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 12 }}>
                {[
                  { icon: '🧩', title: 'Визуальный конструктор', desc: 'Собирайте логику бота блоками на холсте. Соединяйте их — конструктор сам генерирует DSL-код.', color: '#fbbf24' },
                  { icon: '🤖', title: '30+ типов блоков', desc: 'От простого «Ответ» до HTTP-запросов, условий, сценариев, медиа, SQL-запросов и AI-классификации.', color: '#3ecf8e' },
                  { icon: '✨', title: 'AI-генерация схем', desc: 'Опишите задачу — AI предложит готовую структуру бота с нужными блоками и сценариями.', color: '#a78bfa' },
                  { icon: '🛡', title: 'Встроенная валидация', desc: 'DSL-валидатор проверяет схему на ошибки и предупреждения до запуска — экономит время на отладку.', color: '#60a5fa' },
                  { icon: '☁️', title: 'Облачные проекты', desc: 'Сохраняйте проекты в облако и открывайте их с любого устройства. Всё синхронизируется автоматически.', color: '#fbbf24' },
                  { icon: '🎨', title: 'Готовые шаблоны', desc: 'Библиотека шаблонов для квизов, заявок, меню, интернет-магазинов и других типовых задач.', color: '#f87171' },
                  { icon: '📱', title: 'Мобильная версия', desc: 'Адаптивный интерфейс позволяет редактировать схемы с телефона или планшета без потери удобства.', color: '#34d399' },
                  { icon: '⚡', title: 'Быстрый старт', desc: 'Первый рабочий бот за вечер. Установка не нужна — всё работает прямо в браузере.', color: '#fbbf24' },
                  { icon: '🔗', title: 'HTTP-интеграции', desc: 'Подключайте внешние API: блок HTTP-запрос умеет GET/POST с переменными и обработкой ответа.', color: '#0ea5e9' },
                  { icon: '🗄', title: 'Встроенная база данных', desc: 'SQL-блок для хранения данных пользователей, заказов, настроек и любых других сущностей.', color: '#10b981' },
                ].map(({ icon, title, desc, color }) => (
                  <div key={title} style={{ padding: '14px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.025)', border: `1px solid ${color}22`, transition: 'border-color .2s' }} onMouseEnter={e => e.currentTarget.style.borderColor = color + '55'} onMouseLeave={e => e.currentTarget.style.borderColor = color + '22'}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                      <span style={{ fontSize: 22 }}>{icon}</span>
                      <span style={{ fontFamily: 'Syne,system-ui', fontWeight: 700, fontSize: 14, color }}>{title}</span>
                    </div>
                    <p style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.6)', lineHeight: 1.6, margin: 0 }}>{desc}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {page === 'templates' && (
            <div className="lip-scroll" style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '16px' : '24px' }}>
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', lineHeight: 1.65, marginBottom: 20, marginTop: 0 }}>Готовые схемы ботов для быстрого старта. Выберите шаблон, откройте в конструкторе и адаптируйте под свою задачу.</p>
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 12 }}>
                {[
                  ...
                  { icon: '📝', title: 'Сбор заявок', tags: ['Спросить','Сохранить','HTTP'], desc: 'Форма для сбора данных пользователя с сохранением в базу или отправкой на email.', color: '#60a5fa' },
                  { icon: '🛍️', title: 'Интернет-магазин', tags: ['Меню','Кнопки','Оплата'], desc: 'Каталог товаров, корзина, оформление заказа и приём оплаты через Telegram Payments.', color: '#a78bfa' },
                  { icon: '📅', title: 'Запись на приём', tags: ['Спросить','Inline','БД'], desc: 'Выбор даты и времени через inline-кнопки. Запись хранится в базе данных.', color: '#f87171' },
                  ...
                  { icon: '🤝', title: 'Поддержка клиентов', tags: ['Классификация','HTTP','Лог'], desc: 'AI-классификация обращений, автоответы и переадресация сложных вопросов.', color: '#0ea5e9' },
                ].map(({ icon, title, tags, desc, color }) => (
                  <div key={title} style={{ padding: '14px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.08)', transition: 'all .2s', cursor: 'default' }} onMouseEnter={e => { e.currentTarget.style.borderColor = color + '44'; e.currentTarget.style.background = 'rgba(255,255,255,0.04)'; }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'; e.currentTarget.style.background = 'rgba(255,255,255,0.025)'; }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                        <span style={{ fontSize: 24 }}>{icon}</span>
                        <span style={{ fontFamily: 'Syne,system-ui', fontWeight: 700, fontSize: 13, color: '#fff' }}>{title}</span>
                      </div>
                    </div>
                    <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.55)', lineHeight: 1.6, margin: '0 0 10px' }}>{desc}</p>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                      {tags.map(tag => <span key={tag} style={{ fontSize: 10, padding: '2px 8px', borderRadius: 999, background: color + '18', color, border: `1px solid ${color}30`, fontWeight: 600 }}>{tag}</span>)}
                    </div>
                  </div>
                ))}
              </div>
              <div style={{ marginTop: 16, padding: '14px 16px', borderRadius: 12, background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.2)', textAlign: 'center' }}>
                <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', marginBottom: 4 }}>Шаблоны доступны в панели библиотеки внутри конструктора</div>
          ...
          {page === 'docs' && (
            <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
              {/* Sidebar */}
              {!isMobile && (
                <div style={{ width: 190, background: '#111318', borderRight: '1px solid rgba(255,255,255,0.07)', overflowY: 'auto', flexShrink: 0, padding: '12px 8px' }}>
                  {DOC_SECTIONS.map((s, i) => (
                    <button key={i} onClick={() => setDocsSection(i)} style={{ width: '100%', textAlign: 'left', padding: '9px 12px', borderRadius: 8, border: 'none', background: docsSection === i ? 'rgba(251,191,36,0.12)' : 'none', color: docsSection === i ? '#fbbf24' : 'rgba(255,255,255,0.5)', fontSize: 12, fontFamily: 'system-ui', cursor: 'pointer', transition: 'all .15s', marginBottom: 2 }} onMouseEnter={e => { if (docsSection !== i) e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; }} onMouseLeave={e => { if (docsSection !== i) e.currentTarget.style.background = 'none'; }}>
                      {s.label}
                    </button>
                  ))}
                </div>
              )}
              <div className="lip-scroll" style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '14px' : '20px 22px' }}>
                {isMobile && (
                  <div style={{ display: 'flex', gap: 6, overflowX: 'auto', marginBottom: 14, paddingBottom: 6 }}>
                    {DOC_SECTIONS.map((s, i) => (
                      <button key={i} onClick={() => setDocsSection(i)} style={{ flexShrink: 0, padding: '6px 12px', borderRadius: 8, border: `1px solid ${docsSection === i ? 'rgba(251,191,36,0.4)' : 'rgba(255,255,255,0.1)'}`, background: docsSection === i ? 'rgba(251,191,36,0.1)' : 'none', color: docsSection === i ? '#fbbf24' : 'rgba(255,255,255,0.5)', fontSize: 11, cursor: 'pointer', whiteSpace: 'nowrap' }}>{s.label}</button>
                    ))}
                  </div>
                )}
          ...
          {page === 'pricing' && (
            <div className="lip-scroll" style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '16px' : '24px' }}>
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', lineHeight: 1.65, marginBottom: 20, marginTop: 0 }}>Начните бесплатно, а при росте — переходите на Pro. Без скрытых комиссий, без ограничений по времени.</p>
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 14, marginBottom: 20 }}>
                {/* FREE */}
                <div style={{ padding: 20, borderRadius: 16, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.1)' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, marginBottom: 4 }}>
                    <span style={{ fontFamily: 'Syne,system-ui', fontWeight: 800, fontSize: 32, color: '#fff', lineHeight: 1 }}>$0</span>
                    <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', paddingBottom: 4 }}>/мес</span>
                  </div>
                  <div style={{ fontFamily: 'Syne,system-ui', fontWeight: 700, fontSize: 16, color: 'rgba(255,255,255,0.6)', marginBottom: 4 }}>Free</div>
                  <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginBottom: 18 }}>Всегда бесплатно. Навсегда.</div>
                  <div style={{ height: 1, background: 'rgba(255,255,255,0.07)', marginBottom: 16 }} />
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
                    {[
                      ['✅','1 проект в облаке'],
                      ['✅','Все базовые блоки (30+)'],
                      ...
                      <div key={text} style={{ display: 'flex', gap: 10, alignItems: 'center', fontSize: 13, color: ico === '✅' ? 'rgba(255,255,255,0.75)' : 'rgba(255,255,255,0.28)' }}>
                        <span style={{ flexShrink: 0 }}>{ico}</span><span>{text}</span>
                      </div>
                    ))}
                  </div>
                ...
                {/* PRO */}
                <div style={{ padding: 20, borderRadius: 16, background: 'linear-gradient(145deg, rgba(255,215,0,0.07), rgba(255,140,0,0.04))', border: '1px solid rgba(255,215,0,0.3)', position: 'relative', overflow: 'hidden' }}>
                  <div style={{ position: 'absolute', top: 14, right: 14, padding: '3px 10px', borderRadius: 999, background: 'linear-gradient(135deg,#ffd700,#ffaa00)', fontSize: 10, fontWeight: 800, color: '#111', fontFamily: 'Syne,system-ui' }}>ПОПУЛЯРНЫЙ</div>
                  <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, marginBottom: 4 }}>
                    <span style={{ fontFamily: 'Syne,system-ui', fontWeight: 800, fontSize: 32, color: '#ffd700', lineHeight: 1 }}>$8</span>
                    <span style={{ fontSize: 13, color: 'rgba(255,215,0,0.5)', paddingBottom: 4 }}>/мес</span>
                  </div>
                  <div style={{ fontFamily: 'Syne,system-ui', fontWeight: 700, fontSize: 16, color: '#ffd700', marginBottom: 4 }}>Pro</div>
                  <div style={{ fontSize: 12, color: 'rgba(255,215,0,0.45)', marginBottom: 18 }}>Всё для профессиональной работы</div>
                  <div style={{ height: 1, background: 'rgba(255,215,0,0.15)', marginBottom: 16 }} />
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
                    {[
                      ['✅','Безлимитные проекты'],
                      ...
                      <div key={text} style={{ display: 'flex', gap: 10, alignItems: 'center', fontSize: 13, color: 'rgba(255,255,255,0.8)' }}>
                        <span style={{ flexShrink: 0, color: '#3ecf8e' }}>{ico}</span><span>{text}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4, fontFamily: 'Syne,system-ui' }}>Частые вопросы</div>
                {[
                  ...
                  ['Что будет с проектами если перейти с Pro на Free?','Проекты сохранятся. Доступен только 1 проект для редактирования, остальные — только просмотр.'],
                ].map(([q, a]) => (
                  <div key={q} style={{ padding: '12px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)' }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.85)', marginBottom: 5 }}>{q}</div>
                    <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', lineHeight: 1.55 }}>{a}</div>
                  </div>
                ))}
              </div>
            </div>
...
function AuthModal({ tab, setTab, onClose, onLogin, onRegister, canClose = true }) {
  ...
  const iconStyle = { position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'rgba(255,255,255,0.3)', pointerEvents: 'none', display: 'flex', alignItems: 'center' };
  ...
  const fieldInput = (field, extra = {}) => ({
    ...
    color: '#fff',
    border: `1.5px solid ${errors[field] ? '#f87171' : focusedField === field ? '#ffd700' : 'rgba(255,255,255,0.12)'}`,
    borderRadius: 12,
    outline: 'none',
    transition: 'all 0.2s',
    fontFamily: 'var(--mono)',
  ...
  return (
    <div
      style={{
        ...
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        padding: '16px',
        background: 'radial-gradient(ellipse 80% 60% at 50% 50%, rgba(255,180,0,0.07) 0%, rgba(0,0,0,0.96) 70%)',
    ...
    >
      <style>{`
        ...
        @keyframes amSlideUp { from { opacity: 0; transform: translateY(22px) scale(0.97) } to { opacity: 1; transform: translateY(0) scale(1) } }
        @keyframes spin { to { transform: rotate(360deg) } }
        @keyframes pulse-gold { 0%,100% { box-shadow: 0 8px 24px rgba(255,215,0,0.3) } 50% { box-shadow: 0 8px 32px rgba(255,215,0,0.55) } }
        @keyframes regBannerPop { from { opacity: 0; transform: translateY(-12px) scale(0.96) } to { opacity: 1; transform: translateY(0) scale(1) } }
        .am-input-wrap input::placeholder { color: rgba(255,255,255,0.22); }
      `}</style>
      ...
      <div
        style={{
          ...
          maxHeight: '96vh',
          overflowY: 'auto',
          background: 'linear-gradient(160deg, #17191f 0%, #1c1f28 100%)',
          borderRadius: 22,
          border: '1px solid rgba(255,215,0,0.15)',
          boxShadow: '0 40px 80px rgba(0,0,0,0.75), 0 0 60px rgba(255,180,0,0.06), inset 0 1px 0 rgba(255,255,255,0.06)',
          animation: 'amSlideUp 0.32s cubic-bezier(0.34,1.4,0.64,1)',
        }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div style={{ padding: '28px 32px 20px', textAlign: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginBottom: 8 }}>
            <div style={{ width: 48, height: 48, borderRadius: 14, background: 'linear-gradient(135deg,#ffd700,#ff8c00)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, color: '#111', fontWeight: 900, animation: 'pulse-gold 3s ease infinite', flexShrink: 0 }}>◈</div>
            <div style={{ textAlign: 'left' }}>
              <div style={{ fontFamily: 'Syne,system-ui', fontWeight: 800, fontSize: 22, lineHeight: 1.1 }}>
                <span style={{ background: 'linear-gradient(135deg,#ffd700,#ffaa00)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Cicada</span>
                <span style={{ color: 'rgba(255,255,255,0.38)', fontWeight: 400, fontSize: 16, marginLeft: 7 }}>Studio</span>
              </div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginTop: 3, fontFamily: 'system-ui' }}>
        ...
        {screen === 'form' && (
          <div style={{ padding: '0 28px 20px' }}>
            <div style={{ display: 'flex', background: 'rgba(255,255,255,0.04)', borderRadius: 13, padding: 4, border: '1px solid rgba(255,255,255,0.08)' }}>
              {[['login', 'Войти'], ['register', 'Регистрация']].map(([val, label]) => (
                <button
                  key={val}
                  onClick={() => { setTab(val); setErrors({}); setServerError(''); }}
                  style={{ flex: 1, padding: '11px 16px', fontSize: 14, fontWeight: 700, fontFamily: 'Syne,system-ui', background: tab === val ? 'linear-gradient(135deg,#ffd700,#ffaa00)' : 'transparent', color: tab === val ? '#111' : 'rgba(255,255,255,0.42)', border: 'none', borderRadius: 10, cursor: 'pointer', transition: 'all 0.22s', boxShadow: tab === val ? '0 4px 14px rgba(255,215,0,0.3)' : 'none' }}
                >{label}</button>
              ))}
            </div>
            {tab === 'register' && (
        ...
        {screen === 'forgot' && (
          <form onSubmit={handleForgot} style={{ padding: '0 28px 32px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <button type="submit" disabled={loading} style={{ width: '100%', padding: '14px 20px', fontSize: 14, fontWeight: 700, fontFamily: 'Syne,system-ui', background: loading ? 'rgba(255,215,0,0.4)' : 'linear-gradient(135deg,#ffd700,#ffaa00)', color: '#111', border: 'none', borderRadius: 13, cursor: loading ? 'not-allowed' : 'pointer', boxShadow: '0 8px 24px rgba(255,215,0,0.3)' }}>
        ...
        {screen === 'reset' && (
          <form onSubmit={handleReset} style={{ padding: '0 28px 32px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {serverError && <div style={{ padding: '10px 14px', background: 'rgba(248,113,113,0.12)', border: '1px solid rgba(248,113,113,0.35)', borderRadius: 10, fontSize: 12, color: '#f87171', textAlign: 'center' }}>⚠ {serverError}</div>}
            <button type="submit" disabled={loading} style={{ width: '100%', padding: '14px', fontSize: 14, fontWeight: 700, fontFamily: 'Syne,system-ui', background: loading ? 'rgba(255,215,0,0.4)' : 'linear-gradient(135deg,#ffd700,#ffaa00)', color: '#111', border: 'none', borderRadius: 13, cursor: loading ? 'not-allowed' : 'pointer', boxShadow: '0 8px 24px rgba(255,215,0,0.3)' }}>
        ...
        {screen === 'reset-done' && (
          <div style={{ padding: '0 32px 36px', textAlign: 'center' }}>
            ...
            <button onClick={() => { setScreen('form'); setTab('login'); setPassword(''); setConfirmPassword(''); }} style={{ padding: '12px 28px', background: 'linear-gradient(135deg,#ffd700,#ffaa00)', color: '#111', fontWeight: 700, fontSize: 14, border: 'none', borderRadius: 12, cursor: 'pointer' }}>→ Войти</button>
          </div>
        )}
        ...
        {screen === 'form' && (
          <form onSubmit={handleSubmit} style={{ padding: '0 28px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ width: '100%', padding: '14px 20px', fontSize: 15, fontWeight: 700, fontFamily: 'Syne,system-ui', background: loading ? 'rgba(255,215,0,0.35)' : 'linear-gradient(135deg,#ffd700 0%,#f59e0b 100%)', color: '#111', border: 'none', borderRadius: 13, cursor: loading ? 'not-allowed' : 'pointer', transition: 'all 0.22s', boxShadow: loading ? 'none' : '0 8px 24px rgba(255,180,0,0.32)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
              onMouseEnter={e => { if (!loading) { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = '0 12px 32px rgba(255,180,0,0.45)'; } }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = loading ? 'none' : '0 8px 24px rgba(255,180,0,0.32)'; }}
            >
              {loading
                ? <><div style={{ width: 16, height: 16, border: '2.5px solid rgba(0,0,0,0.2)', borderTopColor: '#111', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />{tab === 'login' ? 'Входим...' : 'Создаём...'}</>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 2 }}>
              ...
              <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.25)', fontFamily: 'system-ui', letterSpacing: '0.08em' }}>или</span>
              <div style={{ flex: 1, height: 1, background: 'rgba(255,255,255,0.08)' }} />
            </div>
...
${supportMessage.trim()}`;
  ...
  const inputBase = (field) => ({
    ...
    background: focusedField === field ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.03)',
    color: 'var(--text)',
    border: `1.5px solid ${focusedField === field ? '#ffd700' : 'rgba(255,255,255,0.1)'}`,
  ...
  return (
    ...
    >
      <style>{`
        @keyframes pmSlideIn { from { opacity:0; transform:translateY(12px) scale(0.98) } to { opacity:1; transform:translateY(0) scale(1) } }
        @keyframes pmSlideUp { from { opacity:0; transform:translateY(100%) } to { opacity:1; transform:translateY(0) } }
        @keyframes projectFade { from { opacity: 0; transform: translateX(-6px) } to { opacity: 1; transform: translateX(0) } }
        @keyframes spin2 { to { transform: rotate(360deg) } }
        .pm-nav-btn { display:flex; align-items:center; gap:10px; padding:9px 12px; border-radius:9px; cursor:pointer; border:none; background:none; color:rgba(255,255,255,0.55); font-size:13px; font-family:system-ui,sans-serif; width:100%; text-align:left; transition:all .15s; }
        .pm-nav-btn:hover { background:rgba(255,255,255,0.06); color:rgba(255,255,255,0.9); }
        .pm-nav-btn.pmactive { background:rgba(255,215,0,0.12); color:#ffd700; }
        .pm-action-card:hover { border-color:rgba(255,255,255,0.22) !important; background:rgba(255,255,255,0.05) !important; transform:translateY(-1px); }
        .pm-info-row:hover { border-color:rgba(255,215,0,0.25) !important; }
      ...
      >
        {/* ── LEFT SIDEBAR ── */}
        {!isMobile && (
          <div style={{ width: 224, background: '#111318', borderRight: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
            {/* Logo */}
            <div style={{ padding: '18px 16px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ color: '#ffd700', fontSize: 18, textShadow: '0 0 10px rgba(255,215,0,0.5)' }}>◈</span>
              <span style={{ fontFamily: 'Syne,system-ui', fontWeight: 800, fontSize: 17, background: 'linear-gradient(135deg,#ffd700,#ffaa00)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Cicada</span>
              <span style={{ fontFamily: 'Syne,system-ui', fontSize: 13, color: 'rgba(255,255,255,0.35)', fontWeight: 400 }}>Studio</span>
            </div>
            ...
            <div style={{ padding: '12px 12px 8px' }}>
              <button
                onClick={onClose}
                style={{ width: '100%', padding: '9px 14px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg,#ffd700,#f59e0b)', color: '#111', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'Syne,system-ui', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, boxShadow: '0 4px 16px rgba(245,158,11,0.28)' }}
              >
            ...
            <nav style={{ padding: '4px 8px', display: 'flex', flexDirection: 'column', gap: 2 }}>
              {[
                ...
                { key: 'projects', icon: '📁', label: 'Проекты', badge: projects.length || null },
                ...
                <button key={key} className={`pm-nav-btn${activeTab === key ? ' pmactive' : ''}`} onClick={() => setActiveTab(key)}>
                  ...
                  {badge > 0 && <span style={{ fontSize: 10, fontWeight: 700, padding: '1px 6px', borderRadius: 999, background: 'rgba(255,215,0,0.14)', color: '#ffd700' }}>{badge}</span>}
                </button>
              ))}
            </nav>
            ...
            {/* Secondary nav */}
            <nav style={{ padding: '0 8px', display: 'flex', flexDirection: 'column', gap: 2, flex: 1, overflowY: 'auto' }}>
              {[
                { key: 'docs', icon: '📖', label: 'Документация', action: onOpenInstructions },
                { key: 'support', icon: '🛟', label: 'Поддержка', action: () => setActiveTab('support') },
              ].map(({ key, icon, label, action }) => (
                <button
                  key={key}
                  className={`pm-nav-btn${activeTab === key ? ' pmactive' : ''}`}
                  onClick={action}
                  style={{ opacity: 0.85 }}
                >
                  <span style={{ fontSize: 15 }}>{icon}</span>
            ...
            {/* Pro promo card */}
            {user.plan !== 'pro' && (
              <div style={{ margin: '8px 10px', padding: '14px 12px', borderRadius: 12, background: 'linear-gradient(145deg,rgba(139,92,246,0.22),rgba(109,40,217,0.12))', border: '1px solid rgba(139,92,246,0.28)' }}>
                <div style={{ fontSize: 20, marginBottom: 5 }}>⚡</div>
                <div style={{ fontFamily: 'Syne,system-ui', fontWeight: 700, fontSize: 12, color: '#e9d5ff', lineHeight: 1.35 }}>Разблокируй все<br />возможности</div>
                <button
                  onClick={() => setActiveTab('subscription')}
                  style={{ marginTop: 9, width: '100%', padding: '7px 10px', borderRadius: 8, border: 'none', background: 'rgba(255,255,255,0.1)', color: '#e9d5ff', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'Syne,system-ui', display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center' }}
            ...
            <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 34, height: 34, borderRadius: 9, overflow: 'hidden', background: `linear-gradient(135deg,${avatarColor})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 800, color: 'rgba(0,0,0,0.7)', fontFamily: 'Syne,system-ui', flexShrink: 0 }}>
                {newAvatar ? <img src={newAvatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : avatarLetter}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)', fontFamily: 'Syne,system-ui', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user.name}</div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.32)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'var(--mono)' }}>{user.email}</div>
              </div>
              <button
                onClick={() => { if (confirm('Выйти из аккаунта?')) { onLogout(); onClose(); } }}
                title="Выйти"
                style={{ width: 26, height: 26, borderRadius: 7, background: 'none', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all .15s' }}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {/* Top header */}
          <div style={{ padding: isMobile ? '14px 16px' : '16px 22px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0, background: '#111318' }}>
            <div style={{ position: 'relative', width: isMobile ? 46 : 54, height: isMobile ? 46 : 54, flexShrink: 0 }}>
              <div style={{ width: '100%', height: '100%', borderRadius: 16, overflow: 'hidden', background: `linear-gradient(135deg,${avatarColor})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: isMobile ? 20 : 24, fontWeight: 800, color: 'rgba(0,0,0,0.7)', fontFamily: 'Syne,system-ui' }}>
                {newAvatar ? <img src={newAvatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : avatarLetter}
              </div>
              <button type="button" onClick={() => avatarInputRef.current?.click()} style={{ position: 'absolute', bottom: -2, right: -2, width: 20, height: 20, borderRadius: 6, border: '1.5px solid rgba(255,215,0,0.45)', background: '#111', color: '#ffd700', fontSize: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✎</button>
            </div>

            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'Syne,system-ui', fontWeight: 800, fontSize: isMobile ? 18 : 22, color: '#fff', lineHeight: 1.1 }}>{user.name}</div>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.42)', marginTop: 4, fontFamily: 'var(--mono)', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: isMobile ? 160 : 280 }}>{user.email}</span>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 20, background: 'rgba(62,207,142,0.11)', border: '1px solid rgba(62,207,142,0.25)', fontSize: 9, fontWeight: 700, color: '#3ecf8e', letterSpacing: '0.06em', flexShrink: 0 }}>● ONLINE</span>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center' }}>
              {!isMobile && (
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 14px', borderRadius: 9, border: '1px solid rgba(255,255,255,0.13)', background: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.65)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Syne,system-ui', transition: 'all .15s' }}
              ...
              <button
                onClick={onClose}
                style={{ width: 34, height: 34, borderRadius: 9, border: '1px solid rgba(255,255,255,0.1)', background: 'none', color: 'rgba(255,255,255,0.4)', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .15s' }}
          ...
          {/* Mobile tab bar */}
          {isMobile && (
            <div style={{ display: 'flex', padding: '8px 10px', gap: 6, borderBottom: '1px solid rgba(255,255,255,0.07)', background: '#111318', overflowX: 'auto', flexShrink: 0 }}>
              {[
                { key: 'profile', icon: '👤', label: 'Профиль' },
                ...
                <button
                  ...
                  style={{ flex: '0 0 auto', padding: '7px 14px', borderRadius: 8, border: `1px solid ${activeTab === key ? 'rgba(255,215,0,0.3)' : 'rgba(255,255,255,0.08)'}`, background: activeTab === key ? 'rgba(255,215,0,0.12)' : 'rgba(255,255,255,0.02)', color: activeTab === key ? '#ffd700' : 'rgba(255,255,255,0.5)', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'Syne,system-ui', display: 'flex', alignItems: 'center', gap: 6 }}
                >
            ...
            {activeTab === 'profile' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4,1fr)', gap: 10 }}>
                  {[
                    { label: 'ПРОЕКТОВ', value: projects.length, color: '#ffd700', isNum: true },
                    { label: 'ДНЕЙ С НАМИ', value: user.createdAt ? Math.floor((Date.now() - new Date(user.createdAt)) / 86400000) : '—', color: '#fbbf24', isNum: true },
                    { label: 'ТАРИФ', value: user.plan === 'pro' ? 'PRO' : 'FREE', isPlan: true },
                    { label: 'ПОДДЕРЖКА', value: '24/7', color: '#ffd700', isNum: true },
                  ].map(({ label, value, color, isNum, isPlan }) => (
                    <div key={label} style={{ padding: '13px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.08)' }}>
                      {isPlan ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    <section>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                        {[
                          { icon: '⊕', label: 'Новый проект', sub: 'Создать бота с нуля', color: '#ffd700', action: onClose },
                          { icon: '📖', label: 'Документация', sub: 'Открыть инструкцию', color: '#60a5fa', action: onOpenInstructions },
                          { icon: '🛟', label: 'Поддержка', sub: 'Написать в поддержку', color: '#34d399', action: () => setActiveTab('support') },
                        ].map(({ icon, label, sub, color, action }) => (
                          <button
                            key={label}
                            className="pm-action-card"
                            onClick={action}
                            style={{ textAlign: 'left', cursor: 'pointer', borderRadius: 12, padding: '12px 10px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.09)', transition: 'all .2s' }}
                          >
                            <div style={{ fontSize: 22, color, marginBottom: 8 }}>{icon}</div>
                    ...
                    <section>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                        {[
                          ...
                          { icon: '🌐', label: 'Язык', value: 'Русский', editable: false },
                      ...
                      <div style={{ padding: '16px', borderRadius: 12, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.08)' }}>
                        <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', marginBottom: 14 }}>
                          ...
                          <div style={{ paddingTop: 4 }}>
                            ...
                            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>@ Максимум 2MB</div>
                          </div>
                        </div>
                        <button
                          type="button"
                          ...
                          style={{ width: '100%', padding: '9px 14px', borderRadius: 8, border: '1px solid rgba(255,215,0,0.28)', background: 'rgba(255,215,0,0.07)', color: '#ffd700', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'Syne,system-ui', marginBottom: newAvatar ? 8 : 0, transition: 'all .15s', opacity: avatarSaving ? 0.6 : 1 }}
                    ...
                    <section>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {[
                          ...
                          { icon: '🛡', title: 'Двухфакторная аутентификация', sub: null, subGreen: 'Включена' },
...
function InstructionsContent() {
  const code = (s) => (
    <code style={{ background: 'rgba(255,255,255,0.08)', padding: '2px 7px', borderRadius: 4, fontFamily: 'monospace', fontSize: 12, color: '#3ecf8e' }}>{s}</code>
  );
  const block = (children) => (
    <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', borderRadius: 8, padding: '12px 14px', marginBottom: 12, fontFamily: 'monospace', fontSize: 12, lineHeight: 1.8, color: 'var(--text2)' }}>{children}</div>
  );
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif' }}>
      <h1 style={{ fontSize: 24, color: '#ffd700', marginBottom: 16 }}>🚀 Как пользоваться Cicada Studio</h1>
      <p style={{ marginBottom: 20 }}>Добро пожаловать! Здесь ты собираешь Telegram-бота как из пазлов 🧩</p>

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <h2 style={{ fontSize: 18, color: '#3ecf8e', marginBottom: 12 }}>🧩 1. Добавь блоки</h2>
      <p style={{ marginBottom: 12 }}>Перетащи блоки из левой панели на холст.</p>
      <p style={{ marginBottom: 16 }}><strong>👉 Начни с:</strong></p>
      <ul style={{ marginBottom: 20, paddingLeft: 20 }}>
        <li><strong>Версия</strong></li>
        <li><strong>Бот</strong> — обязательно укажи токен</li>
        <li><strong>Старт</strong></li>
      </ul>

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <h2 style={{ fontSize: 18, color: '#3ecf8e', marginBottom: 12 }}>🔗 2. Соединяй блоки</h2>
      <p style={{ marginBottom: 12 }}>Соединяй их сверху вниз — как конструктор.</p>
      <p style={{ marginBottom: 8 }}><strong>Пример:</strong></p>
      <p style={{ marginBottom: 20, color: '#60a5fa' }}>Старт → Ответ → Кнопки</p>

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <h2 style={{ fontSize: 18, color: '#3ecf8e', marginBottom: 12 }}>✏️ 3. Настрой блок</h2>
      <p style={{ marginBottom: 12 }}>Нажми на блок и задай:</p>
      <ul style={{ marginBottom: 20, paddingLeft: 20 }}>
        <li>текст</li>
        <li>команду</li>
        <li>переменные</li>
      ...
      <h2 style={{ fontSize: 18, color: '#3ecf8e', marginBottom: 12 }}>⚡ 4. Добавь логику</h2>
      <p style={{ marginBottom: 12 }}>Используй:</p>
      <ul style={{ marginBottom: 20, paddingLeft: 20 }}>
        <li><strong>Если</strong> — условия</li>
        <li><strong>Спросить</strong> — ввод данных</li>
        <li><strong>Сохранить</strong> — память</li>
      </ul>

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <h2 style={{ fontSize: 18, color: '#3ecf8e', marginBottom: 12 }}>🎮 5. Обрабатывай действия</h2>
      <p style={{ marginBottom: 12 }}>Добавь:</p>
      <ul style={{ marginBottom: 20, paddingLeft: 20 }}>
        <li>Команды ({code('/help')})</li>
        <li>Нажатия кнопок</li>
        <li>Сценарии</li>
      </ul>

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <h2 style={{ fontSize: 18, color: '#3ecf8e', marginBottom: 12 }}>📜 6. Смотри код справа</h2>
      <p style={{ marginBottom: 20 }}>Код генерируется автоматически — ничего писать не нужно.</p>

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <h2 style={{ fontSize: 18, color: '#3ecf8e', marginBottom: 12 }}>✅ 7. Проверь и сохрани</h2>
      <p style={{ marginBottom: 8 }}>Нажми:</p>
      <p style={{ marginBottom: 20 }}>✔ <strong>Проверить</strong><br/>💾 <strong>Сохранить проект</strong></p>

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <h2 style={{ fontSize: 18, color: '#f59e0b', marginBottom: 12 }}>💡 Пример</h2>
      <p style={{ marginBottom: 8 }}>Старт → Ответ → Кнопки</p>
      <p style={{ marginBottom: 8 }}>Команда → Ответ</p>
      <p style={{ marginBottom: 20 }}>Нажатие → Ответ</p>

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <h2 style={{ fontSize: 20, color: '#60a5fa', marginBottom: 16 }}>🖥️ Локальный запуск бота</h2>
      <p style={{ marginBottom: 16, color: 'var(--text2)' }}>
        Cicada Studio генерирует <code style={{ color: '#ffd700' }}>.ccd</code>-файл, который запускается через утилиту <strong>cicada</strong> на твоём компьютере или сервере.
      </p>

      <h3 style={{ fontSize: 15, color: '#f59e0b', marginBottom: 10 }}>📋 Требования</h3>
      <ul style={{ marginBottom: 16, paddingLeft: 20, lineHeight: 2 }}>
        <li><strong>Python 3.10+</strong> (рекомендуется последняя версия)</li>
        <li>pip (входит в Python)</li>
        <li>Telegram Bot Token (получить у <strong>@BotFather</strong>)</li>
      </ul>

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <h3 style={{ fontSize: 15, color: '#60a5fa', marginBottom: 10 }}>🪟 Windows</h3>
      <p style={{ marginBottom: 8, color: 'var(--text2)' }}>Установи Python с <strong>python.org</strong>, убедись что галочка <em>«Add Python to PATH»</em> стоит. Затем в командной строке (cmd или PowerShell):</p>
      {block(<>
        <div style={{ color: '#94a3b8' }}># Глобально (рекомендуется)</div>
        <div>pip install cicada-tg</div>
        <br/>
        <div style={{ color: '#94a3b8' }}># Или в виртуальном окружении</div>
        <div>python -m venv venv</div>
        <div>venv\Scriptsctivate</div>
        <div>pip install cicada-tg</div>
      </>)}

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <h3 style={{ fontSize: 15, color: '#60a5fa', marginBottom: 10 }}>🐧 Linux / macOS</h3>
      {block(<>
        <div style={{ color: '#94a3b8' }}># Вариант 1 — с флагом (если pip ругается)</div>
        <div>pip install cicada-tg --break-system-packages</div>
        <br/>
        <div style={{ color: '#94a3b8' }}># Вариант 2 — в виртуальном окружении</div>
      ...
      <h3 style={{ fontSize: 15, color: '#3ecf8e', marginBottom: 10 }}>▶ Запуск</h3>
      {block(<>
        <div style={{ color: '#94a3b8' }}># Справка по командам</div>
        <div>cicada</div>
        <br/>
        <div style={{ color: '#94a3b8' }}># Запустить бота из файла</div>
        <div>cicada bot.ccd</div>
      </>)}
      <p style={{ marginTop: 10, marginBottom: 20, color: 'var(--text2)', fontSize: 13 }}>
        Скачай <code style={{ color: '#ffd700' }}>.ccd</code>-файл кнопкой <strong>↓ .ccd</strong> в правой панели и запусти командой выше.
      </p>

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <h2 style={{ fontSize: 18, color: '#ef4444', marginBottom: 12 }}>⚠️ Важно</h2>
      <ul style={{ marginBottom: 20, paddingLeft: 20 }}>
        <li>Блоки должны быть соединены</li>
        <li>Должен быть блок <strong>Старт</strong></li>
        <li>Кнопка <strong>Запустить</strong> активна только когда в блоке <strong>Бот</strong> указан токен</li>
      </ul>

      <hr style={{ border: '1px solid var(--border)', margin: '20px 0' }} />

      <p style={{ fontSize: 16, color: '#ffd700', textAlign: 'center', marginTop: 24 }}>🎉 Готово! Теперь ты можешь собрать своего бота</p>
    </div>
  );
}
