...
`
  ...
  const authModalNode = showAuthModal ? (
    <AuthModal
      ...
      onLogin={async (email, password, tgData, passkeyMode = false) => {
        let user;
        if (tgData) {
            ...
            <div style={{ display:'grid', gridTemplateColumns: lp ? '1fr' : '1fr 1.05fr', gap: lp ? 24 : 40, alignItems:'flex-start', width:'100%' }}>
              ...
              <div className="lp-fadeup">
                ...
                <div style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'center', marginBottom:14 }}>
                  ...
                  <button className="lp-btn-ghost" style={{ borderRadius:10, padding:'10px 20px', fontSize:14, display:'flex', alignItems:'center', gap:7 }}>
                    <span style={{ width:26, height:26, borderRadius:'50%', border:'1.5px solid rgba(255,255,255,0.35)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                      <svg viewBox="0 0 12 12" width="10" height="10"><polygon points="3,1 11,6 3,11" fill="rgba(255,255,255,0.85)"/></svg>
                    </span>
                    Смотреть демо
                  </button>
                </div>
...
function AuthModal({ tab, setTab, onClose, onLogin, onRegister, canClose = true }) {
  ...
  const iconStyle = { position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: 'rgba(99,102,241,0.7)', pointerEvents: 'none', display: 'flex', alignItems: 'center' };

  const fieldInput = (field, extra = {}) => ({
    width: '100%',
    padding: '14px 14px 14px 44px',
    fontSize: 14,
    background: focusedField === field ? 'rgba(59,130,246,0.08)' : 'rgba(10,8,28,0.8)',
    color: '#fff',
    border: `1.5px solid ${errors[field] ? '#f87171' : focusedField === field ? 'rgba(99,102,241,0.8)' : 'rgba(99,102,241,0.3)'}`,
    borderRadius: 10,
    outline: 'none',
    transition: 'all 0.2s',
    fontFamily: "'JetBrains Mono','Courier New',monospace",
    boxSizing: 'border-box',
    boxShadow: focusedField === field ? '0 0 12px rgba(99,102,241,0.25)' : 'none',
    ...extra,
  });
  ...
  return (
    <div
      style={{
        ...
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        padding: '16px',
        background: 'radial-gradient(ellipse 90% 80% at 50% 40%, rgba(99,40,240,0.18) 0%, rgba(6,3,15,0.97) 65%)',
    ...
    >
      <style>{`
        ...
        @keyframes pulse-gold { 0%,100% { box-shadow: 0 0 18px rgba(255,160,0,0.4),0 0 40px rgba(255,100,0,0.2) } 50% { box-shadow: 0 0 28px rgba(255,160,0,0.65),0 0 60px rgba(255,100,0,0.35) } }
        ...
        .am-input-wrap input::placeholder { color: rgba(255,255,255,0.25); font-family:'JetBrains Mono','Courier New',monospace; }
        .am-input-wrap input { caret-color: #f97316; }
        .am-tab-active { background: linear-gradient(135deg,#f97316,#dc2626) !important; color: #fff !important; box-shadow: 0 4px 18px rgba(249,115,22,0.45) !important; }
      `}</style>
      ...
      <div
        style={{
          ...
          maxHeight: '96vh',
          overflowY: 'auto',
          background: 'linear-gradient(160deg, #0d0920 0%, #0a0618 60%, #080515 100%)',
          borderRadius: 18,
          border: '1px solid rgba(249,115,22,0.35)',
          boxShadow: '0 0 0 1px rgba(99,40,240,0.2), 0 40px 80px rgba(0,0,0,0.85), 0 0 60px rgba(249,115,22,0.1), 0 0 100px rgba(99,40,240,0.12)',
      ...
      >
        ...
        {[['0','0','borderTop','borderLeft'],['0','auto','borderTop','borderRight'],['auto','0','borderBottom','borderLeft'],['auto','auto','borderBottom','borderRight']].map(([t,r,b1,b2],i) => (
          <div key={i} style={{ position:'absolute', top:t!=='auto'?0:'auto', right:r!=='auto'?0:'auto', bottom:r==='auto'&&t==='auto'?0:'auto', left:r==='0'&&t!=='auto'?0:r==='auto'&&t==='auto'?0:'auto', width:22, height:22, borderTop: (b1==='borderTop'||b2==='borderTop') ? '2px solid #f97316' : 'none', borderBottom: (b1==='borderBottom'||b2==='borderBottom') ? '2px solid #f97316' : 'none', borderLeft: (b1==='borderLeft'||b2==='borderLeft') ? '2px solid #f97316' : 'none', borderRight: (b1==='borderRight'||b2==='borderRight') ? '2px solid #f97316' : 'none', borderRadius: i===0?'18px 0 0 0':i===1?'0 18px 0 0':i===2?'0 0 0 18px':'0 0 18px 0', animation:'cornerGlow 2.5s ease-in-out infinite', animationDelay:`${i*0.4}s` }} />
        ))}
        {/* Header */}
        <div style={{ padding: '32px 32px 22px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 6 }}>
            <div style={{ flexShrink: 0, lineHeight: 1 }}>
              <img src={cicadaLogo} alt="Cicada Studio" style={{ width: 72, height: 72, objectFit: 'cover', borderRadius: 14, boxShadow: '0 0 20px rgba(255,180,0,0.35)' }} />
            </div>
            <div style={{ textAlign: 'left' }}>
              <div style={{ fontFamily: 'Syne,system-ui', fontWeight: 800, fontSize: 24, lineHeight: 1.1, letterSpacing: '-0.01em' }}>
                <span style={{ background: 'linear-gradient(90deg,#06b6d4,#818cf8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>CiCCaDa</span>
                <span style={{ color: 'rgba(255,255,255,0.55)', fontWeight: 400, fontSize: 16, marginLeft: 8 }}>Studio</span>
              </div>
            </div>
          </div>
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)', fontFamily: 'system-ui', letterSpacing: '0.02em' }}>
            Добро пожаловать
          </div>
        </div>
        ...
        {screen === 'form' && (
          <div style={{ padding: '0 28px 20px' }}>
            <div style={{ display: 'flex', background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: 4, border: '1px solid rgba(249,115,22,0.25)' }}>
              {[['login', 'Войти'], ['register', 'Регистрация']].map(([val, label]) => (
                <button
                  key={val}
                  onClick={() => { setTab(val); setErrors({}); setServerError(''); }}
                  style={{ flex: 1, padding: '11px 16px', fontSize: 14, fontWeight: 700, fontFamily: 'Syne,system-ui', background: tab === val ? 'linear-gradient(135deg,#f97316,#dc2626)' : 'transparent', color: tab === val ? '#fff' : 'rgba(255,255,255,0.45)', border: 'none', borderRadius: 9, cursor: 'pointer', transition: 'all 0.22s', boxShadow: tab === val ? '0 4px 18px rgba(249,115,22,0.45)' : 'none' }}
                >{label}</button>
              ))}
            </div>
            {tab === 'register' && (
        ...
        {screen === 'form' && (
          <form onSubmit={handleSubmit} style={{ padding: '0 28px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ width: '100%', padding: '15px 20px', fontSize: 15, fontWeight: 700, fontFamily: 'Syne,system-ui', background: loading ? 'rgba(249,115,22,0.35)' : 'linear-gradient(135deg,#f97316 0%,#dc2626 100%)', color: '#fff', border: 'none', borderRadius: 12, cursor: loading ? 'not-allowed' : 'pointer', transition: 'all 0.22s', boxShadow: loading ? 'none' : '0 8px 28px rgba(249,115,22,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, letterSpacing: '0.02em' }}
              onMouseEnter={e => { if (!loading) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 14px 36px rgba(249,115,22,0.6)'; } }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = loading ? 'none' : '0 8px 28px rgba(249,115,22,0.45)'; }}
            >
              {loading
            ...
            {tab === 'login' && (
              <button
                ...
                disabled={loading}
                style={{ width: '100%', padding: '13px 18px', fontSize: 14, fontWeight: 700, fontFamily: 'Syne,system-ui', background: 'linear-gradient(135deg,#7c3aed,#2563eb)', color: '#fff', border: 'none', borderRadius: 12, cursor: loading ? 'not-allowed' : 'pointer', boxShadow: '0 8px 24px rgba(37,99,235,0.28)' }}
              >
                🔐 Войти по passkey
              </button>
            )}
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{ padding: isMobile ? '14px 16px' : '16px 22px', borderBottom: '1px solid rgba(249,115,22,0.18)', display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0, background: 'linear-gradient(90deg,rgba(13,9,32,0.95),rgba(8,6,18,0.95))', position: 'relative', backdropFilter: 'blur(8px)' }}>
            ...
            <div style={{ position: 'relative', width: isMobile ? 46 : 56, height: isMobile ? 46 : 56, flexShrink: 0 }}>
              ...
              <button type="button" onClick={() => avatarInputRef.current?.click()} style={{ position: 'absolute', bottom: -2, right: -2, width: 20, height: 20, borderRadius: 6, border: '1.5px solid rgba(249,115,22,0.6)', background: '#0d0920', color: '#f97316', fontSize: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 8px rgba(249,115,22,0.3)' }}>✎</button>
            </div>
          ...
          {/* Mobile tab bar */}
          {isMobile && (
            <div style={{ display: 'flex', padding: '8px 10px', gap: 6, borderBottom: '1px solid rgba(249,115,22,0.18)', background: 'linear-gradient(90deg,rgba(13,9,32,0.98),rgba(8,6,18,0.98))', overflowX: 'auto', flexShrink: 0 }}>
              {[
                { key: 'profile', icon: '👤', label: 'Профиль' },
                ...
                <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 14, padding: 16 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div style={{ padding: '12px 14px', borderRadius: 12, background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.24)' }}>
                      <div style={{ fontSize: 13, fontWeight: 800, color: '#ddd6fe', fontFamily: 'Syne, system-ui', marginBottom: 6 }}>🔐 Вход по passkey</div>
