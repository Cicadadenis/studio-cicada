...
1) ИНТЕНТ — что делает бот (викторина, заказ, поддержка), цель и точка входа: /start, команда или первая кнопка.
2) СОСТОЯНИЯ / ЭКРАНЫ — логические узлы (приветствие, вопрос_1, итог). У каждого экрана с действием — ясный триггер: start, command, callback; текст кнопки в точности совпадает с callback.label.
3) СОБЫТИЯ И ПЕРЕХОДЫ — из каждого экрана с кнопками определи, куда ведёт каждая кнопка: отдельный стек с корнем callback или run(сценарий). Не оставляй незавершённых веток.
4) УСЛОВИЯ — ветвления через condition и else; многократные правильные/неправильные ответы могут сходиться в общий следующий экран (reuse через run).
5) ДАННЫЕ И СЧЁТ — любая {переменная} в message/save/condition объявляй раньше в том же стеке (ask/get/remember/http) либо это системная переменная. Счёт между разными стеками callback надёжно вести через save/get с одним ключом, например "викторина_счёт_{chat_id}" (в JSON value для чисел — строка или число по смыслу; после get переменная объявлена).
6) СБОРКА JSON — s0 только bot; координаты и id по правилам ниже; после run в стеке start/command/callback нет stop.

Промежуточный план (интент, блоки, переходы) не печатай — только итоговый JSON.
...
СЦЕНАРИЙ И НЕСКОЛЬКО ВОПРОСОВ:
  Если нужно спросить 2+ значения (например имя+город или логин+пароль), используй scenario+step — это самый надёжный FSM-паттерн:
  s1: start/callback→message→run(имя)  ← БЕЗ stop после run | s2: scenario(имя)→step(ш1)→ask→step(ш2)→ask→message/condition→stop
  После последнего ask ОБЯЗАТЕЛЬНО добавь видимый ответ пользователю (message/buttons/condition с ответами), иначе бот выглядит зависшим.
...
4. Не ставь несколько ask подряд в start/callback/command. Для 2+ вопросов используй scenario+step и запускай его через run без stop после run.
...
═══ ЭТАЛОН: ТЕКСТ В БД (save / get) ═══
Если нужно «ввести строку → сохранить → прочитать», собирай JSON так (имена кнопок = точный callback.label):

  • старт: message → buttons ("💾 Сохранить", "📖 Прочитать") → stop
  • callback "💾 Сохранить": ask(question:"✏️ Введите значение:", varname:"значение") → save(key:"моё_поле", value:"{значение}") → message("✅ Сохранено!") → buttons → stop
  • callback "📖 Прочитать": get(key:"моё_поле", varname:"значение") → condition(cond:"не значение") → message("📭 Ничего не сохранено") → buttons("💾 Сохранить") → else → message("📄 Значение: {значение}") → buttons → stop

Ключ в save.key и get.key должен совпадать символ в символ (например одно и то же "моё_поле"). Условие «пусто в БД»: после get используй condition с cond:"не значение" (или другое имя переменной из get.varname).

═══ ЭТАЛОН: DROPBOX / ФАЙЛ (file_id в БД, один пользователь — один ключ) ═══
Запросы вроде «Dropbox», «загрузить файл», «хранилище» — этот паттерн (не смешивать с медиа-триггером document_received, если в ТЗ явно сценарий по кнопке):

  • старт: message → buttons ("Загрузить файл", "Просмотреть файлы") → stop
  • callback "Загрузить файл": ТОЛЬКО run(name:"загрузка") — БЕЗ stop в этом стеке
  • стек scenario "загрузка": step → ask(question:"Отправьте файл для загрузки:", varname:"файл") → save(key:"файл_{chat_id}", value:"{файл}") → message("Файл загружен!") → stop
    Все блоки ask, save, message идут подряд в ОДНОМ шаге; НЕ вставляй лишний step между ask и save.
  • callback "Просмотреть файлы": ТОЛЬКО run(name:"просмотр") — БЕЗ stop
  • стек scenario "просмотр": step → get(key:"файл_{chat_id}", varname:"файлы") → condition(cond:"не файлы") → message("📭 Ничего не сохранено") → buttons("Загрузить файл") → else → message("Ваши файлы: {файлы}") → buttons → stop

Строка ключа в save и get ОБЯЗАНА совпадать (например обе "файл_{chat_id}"). Подстановка {chat_id} в ключе — только вид "файл_{chat_id}", не опечатывай закрывающую }.

Имя переменной для ask (varname) — своё (например "файл"); в save.value используй именно {файл}, не {файл_id}, если корень стека — scenario (не document_received).`;
...
const FEW_SHOT_USER_5 = `бот Dropbox: приветствие, кнопки Загрузить файл и Просмотреть файлы; по загрузке спросить файл, сохранить Telegram file_id в БД под ключом с chat_id; по просмотру показать сохранённое или что пусто`;
const FEW_SHOT_ASSISTANT_5 = `[{"id":"s0","x":40,"y":40,"blocks":[{"id":"b0","type":"bot","props":{"token":"YOUR_BOT_TOKEN"}}]},{"id":"s1","x":400,"y":40,"blocks":[{"id":"b1","type":"start","props":{}},{"id":"b2","type":"message","props":{"text":"Добро пожаловать в Drop Box! 📦"}},{"id":"b3","type":"buttons","props":{"rows":"Загрузить файл, Просмотреть файлы"}},{"id":"b4","type":"stop","props":{}}]},{"id":"s2","x":760,"y":40,"blocks":[{"id":"b5","type":"callback","props":{"label":"Загрузить файл"}},{"id":"b6","type":"run","props":{"name":"загрузка"}}]},{"id":"s3","x":1120,"y":40,"blocks":[{"id":"b7","type":"scenario","props":{"name":"загрузка"}},{"id":"b8","type":"step","props":{"name":"загрузка_файла"}},{"id":"b9","type":"ask","props":{"question":"Отправьте файл для загрузки:","varname":"файл"}},{"id":"b10","type":"save","props":{"key":"файл_{chat_id}","value":"{файл}"}},{"id":"b11","type":"message","props":{"text":"Файл загружен! 📦"}},{"id":"b12","type":"stop","props":{}}]},{"id":"s4","x":400,"y":380,"blocks":[{"id":"b13","type":"callback","props":{"label":"Просмотреть файлы"}},{"id":"b14","type":"run","props":{"name":"просмотр"}}]},{"id":"s5","x":760,"y":380,"blocks":[{"id":"b15","type":"scenario","props":{"name":"просмотр"}},{"id":"b16","type":"step","props":{"name":"просмотр_файлов"}},{"id":"b17","type":"get","props":{"key":"файл_{chat_id}","varname":"файлы"}},{"id":"b18","type":"condition","props":{"cond":"не файлы"}},{"id":"b19","type":"message","props":{"text":"📭 Ничего не сохранено"}},{"id":"b20","type":"buttons","props":{"rows":"Загрузить файл"}},{"id":"b21","type":"else","props":{}},{"id":"b22","type":"message","props":{"text":"Ваши файлы: {файлы}"}},{"id":"b23","type":"stop","props":{}}]}]`;

const FEW_SHOT_USER_6 = `бот сохраняет текст в базу: кнопка Сохранить спрашивает значение и пишет в поле моё_поле, кнопка Прочитать показывает или пусто`;
const FEW_SHOT_ASSISTANT_6 = `[{"id":"s0","x":40,"y":40,"blocks":[{"id":"b0","type":"bot","props":{"token":"YOUR_BOT_TOKEN"}}]},{"id":"s1","x":400,"y":40,"blocks":[{"id":"b1","type":"start","props":{}},{"id":"b2","type":"message","props":{"text":"Привет!"}},{"id":"b3","type":"buttons","props":{"rows":"💾 Сохранить, 📖 Прочитать"}},{"id":"b4","type":"stop","props":{}}]},{"id":"s2","x":760,"y":40,"blocks":[{"id":"b5","type":"callback","props":{"label":"💾 Сохранить"}},{"id":"b6","type":"ask","props":{"question":"✏️ Введите значение:","varname":"значение"}},{"id":"b7","type":"save","props":{"key":"моё_поле","value":"{значение}"}},{"id":"b8","type":"message","props":{"text":"✅ Сохранено!"}},{"id":"b9","type":"buttons","props":{"rows":"📖 Прочитать, 🏠 Главная"}},{"id":"b10","type":"stop","props":{}}]},{"id":"s3","x":1120,"y":40,"blocks":[{"id":"b11","type":"callback","props":{"label":"📖 Прочитать"}},{"id":"b12","type":"get","props":{"key":"моё_поле","varname":"значение"}},{"id":"b13","type":"condition","props":{"cond":"не значение"}},{"id":"b14","type":"message","props":{"text":"📭 Ничего не сохранено"}},{"id":"b15","type":"buttons","props":{"rows":"💾 Сохранить"}},{"id":"b16","type":"else","props":{}},{"id":"b17","type":"message","props":{"text":"📄 Значение: {значение}"}},{"id":"b18","type":"buttons","props":{"rows":"💾 Обновить, 🏠 Главная"}},{"id":"b19","type":"stop","props":{}}]},{"id":"s4","x":400,"y":380,"blocks":[{"id":"b20","type":"callback","props":{"label":"🏠 Главная"}},{"id":"b21","type":"message","props":{"text":"Привет!"}},{"id":"b22","type":"buttons","props":{"rows":"💾 Сохранить, 📖 Прочитать"}},{"id":"b23","type":"stop","props":{}}]},{"id":"s5","x":760,"y":380,"blocks":[{"id":"b24","type":"callback","props":{"label":"💾 Обновить"}},{"id":"b25","type":"ask","props":{"question":"✏️ Введите значение:","varname":"значение"}},{"id":"b26","type":"save","props":{"key":"моё_поле","value":"{значение}"}},{"id":"b27","type":"message","props":{"text":"✅ Сохранено!"}},{"id":"b28","type":"buttons","props":{"rows":"📖 Прочитать, 🏠 Главная"}},{"id":"b29","type":"stop","props":{}}]}]`;
