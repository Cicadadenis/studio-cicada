...
// Блоки, которые AI может вернуть в JSON — СТРОГИЙ whitelist (совпадает с AI_SYSTEM_PROMPT).
// JSON компилируется в DSL через generateDSL; неизвестный type отсекаем до генерации.
...
function hintForUndeclaredVar(varName) {
  if (MEDIA_ONLY_VAR_NAMES.has(varName)) {
    return (
      `В строгом режиме AI медиа-триггеров нет — не используй {файл_id}. ` +
...
Разрешены ТОЛЬКО следующие типы блоков:
  bot, start, callback, scenario, step, message, buttons, ask, remember, get, condition, else, run, stop

Соответствие инструкциям Cicada в DSL:
  бот→bot · при старте→start · при нажатии→callback · сценарий→scenario · шаг→step · ответ→message · кнопки→buttons · спросить→ask · запомнить→remember · получить→get · если→condition · иначе→else · запустить→run · стоп→stop

Запрещено придумывать новые типы (command, goto, inline, save, http, pause, медиа-триггеры и т.д.).
Если в задаче нужна возможность, которой нет в списке — используй ближайший эквивалент из whitelist.
...
5) ДАННЫЕ — любая {переменная} в message/condition объявлена выше ask/get/remember в том же стеке или это системная переменная. Без save: счёт между визитами «запоминается» только через редактор/БД пользователя — здесь только remember/get в рамках whitelist; если «хранилище» просят, моделируй упрощённо (remember + message) или только подтверждение ввода.
...
ВНУТРИ СТЕКОВ:
  ...
  get        {key:"ключ", varname:"результат"} — прочитать из KV (ключ как строковый литерал)
  condition  {cond:"..."}                     — если
  else       {}                               — иначе
...
ЗАПРЕЩЕНО ссылаться на {переменная} в message/condition, если она не объявлена выше в этом стеке и не системная.

═══ ФАЙЛ ОТ ПОЛЬЗОВАТЕЛЯ (без медиа-триггеров) ═══

В whitelist нет document_received — принимай файл только через ask в сценарии: пользователь может прислать документ, бот кладёт file_id в varname.
Пример: step → ask("Пришлите файл", varname:"файл") → message("Принято, id: {файл}") → stop
...
const FEW_SHOT_USER_5 = `бот для приёма файла: приветствие, кнопка «Отправить файл», сценарий просит файл и отвечает подтверждением с подстановкой {файл} (Telegram file_id), без сохранения в базу`;
const FEW_SHOT_ASSISTANT_5 = `[{"id":"s0","x":40,"y":40,"blocks":[{"id":"b0","type":"bot","props":{"token":"YOUR_BOT_TOKEN"}}]},{"id":"s1","x":400,"y":40,"blocks":[{"id":"b1","type":"start","props":{}},{"id":"b2","type":"message","props":{"text":"Привет! Могу принять файл."}},{"id":"b3","type":"buttons","props":{"rows":"📎 Отправить файл"}},{"id":"b4","type":"stop","props":{}}]},{"id":"s2","x":760,"y":40,"blocks":[{"id":"b5","type":"callback","props":{"label":"📎 Отправить файл"}},{"id":"b6","type":"run","props":{"name":"приём_файла"}}]},{"id":"s3","x":1120,"y":40,"blocks":[{"id":"b7","type":"scenario","props":{"name":"приём_файла"}},{"id":"b8","type":"step","props":{"name":"ждём_файл"}},{"id":"b9","type":"ask","props":{"question":"Пришлите документ или фото файлом:","varname":"файл"}},{"id":"b10","type":"message","props":{"text":"✅ Принято. Идентификатор файла: {файл}"}},{"id":"b11","type":"stop","props":{}}]}]`;
