...
class Executor:
    ...
    def _exec_body(self, stmts: list, ctx, start_index: int = 0) -> bool:
        ...
        signal = None
        try:
            for idx in range(start_index, len(stmts)):
                stmt = stmts[idx]
    ...
    def _exec_save_to_db(self, stmt: SaveToDB, ctx):
        value = self._resolve_val(stmt.value, ctx)
        get_db().set(str(ctx.user_id), self._resolve_db_key(stmt.key, ctx), value)

    def _exec_load_from_db(self, stmt: LoadFromDB, ctx):
        value = get_db().get(str(ctx.user_id), self._resolve_db_key(stmt.key, ctx))
