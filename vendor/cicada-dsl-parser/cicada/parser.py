...
Синтаксис поддерживается:
    ...
    спросить "вопрос" → переменная   (стрелка также ->)
    ...
    картинка "url"
    стикер "file_id"
    переключить переменная: "вариант": ...
"""
...
@dataclass
class SwitchStmt:
    """переключить var: \"литерал\": тело … (первое совпадение со значением переменной)"""
    variable: str
    cases: list  # list[tuple[str, list]]
...
class InlineKeyboardFromList:
    """Динамическая inline-клавиатура из списка словарей/объектов."""
    items: Any
    text_field: str
    id_field: str
    callback_prefix: str = ""
...
def _normalize_unicode_math_ops_for_expr(src: str) -> str:
    """«÷» / «×» вне строк → «/» / «*» (совместимо с токенизатором выражений)."""
    out = []
    i = 0
    in_single = False
    in_double = False
...
def parse_expr(raw: str, lineno: int | None = None):
    """
    ...
    """
    tokens = _tokenize_expr(_normalize_unicode_math_ops_for_expr(raw.strip()), lineno=lineno)
    if not tokens:
        return Literal("")
    ...
    try:
        ...
        if ep.pos < len(tokens):
            ...
            _raise_syntax(
                f"Непарсированный остаток выражения: {remaining!r}", lineno
            )
        return node
    except SyntaxError as e:
        if lineno is not None and (e.lineno is None or e.lineno == 0):
            e.lineno = lineno
        raise


def _raise_syntax(msg: str, lineno: int | None = None) -> None:
    """SyntaxError с номером строки в исходнике (для lint и ответов API)."""
    if lineno is not None:
        raise SyntaxError(msg, (None, int(lineno), 0, None))
    raise SyntaxError(msg)


def _ensure_balanced_braces_interpolation(
    s: str, *, ctx: str = "строка", lineno: int | None = None
) -> None:
    """Проверяет парность { … } в шаблонах (ловит «{chat_id)» вместо «{chat_id}»)."""
    depth = 0
    for i, c in enumerate(s):
        if c == "{":
            ...
            if depth < 0:
                _raise_syntax(
                    f"{ctx}: лишняя закрывающая «}}» во фрагменте ключа/шаблона (симв. {i + 1})",
                    lineno,
                )
    if depth != 0:
        _raise_syntax(
            f"{ctx}: непарные фигурные скобки — проверьте «{{имя}}», а не «{{имя)» и т.п.",
            lineno,
        )


def _validate_quoted_template_fragment(
    s: str, ctx: str, lineno: int | None = None
) -> None:
    if "{" in s or "}" in s:
        _ensure_balanced_braces_interpolation(s, ctx=ctx, lineno=lineno)


def _tokenize_expr(src: str, lineno: int | None = None) -> list:
    """Разбивает строку выражения на токены. Вся строка должна разбираться (нет «тихих» пропусков)."""
    src = src.strip()
    if not src:
        return []
    token_re = re.compile(
        ...
        r'|\*\*|//|>=|<=|==|!=|&&|\|\|'                  # двойные операторы
        r'|[+\-*/%><!(),\[\]{}]'                       # одиночные операторы, включая {{ }}
        r'|\s+'                                         # пробелы
    )
    tokens: list = []
    pos = 0
    for m in token_re.finditer(src):
        if m.start() > pos and src[pos:m.start()].strip():
            bad = src[pos:m.start()].strip()
            _raise_syntax(f"Недопустимые символы в выражении: {bad!r}", lineno)
        tok = m.group()
        if tok.strip():
            tokens.append(tok)
        pos = m.end()
...
class _ExprParser:
    ...
    def parse_atom(self):
        ...
        # { выражение } — тот же синтаксис, что в шаблонах из ИИ («{файл}»)
        if t == "{":
            self.consume("{")
            node = self.parse_or()
            self.consume("}")
...
def tokenise(text: str):
    """Разбивает исходник на значимые строки (без комментариев).

    Возвращает (строки, номер_строки_в_файле_для_каждой) — номер 1-based по полному тексту,
    чтобы диагностика совпадала с редактором.
    """
    lines: list = []
    line_nos: list = []
...
def parse_string_expr(raw: str, lineno: int | None = None) -> list:
    """
    ...
    if stripped_quote and ('{' in stripped_quote or '}' in stripped_quote):
        _ensure_balanced_braces_interpolation(
            stripped_quote, ctx="шаблон в строке", lineno=lineno
        )
        parts = []
        pattern = r'\{([^}]+)\}'
        last_end = 0
        for match in re.finditer(pattern, stripped_quote):
            ...
            try:
                parts.append(parse_expr(inner, lineno=lineno))
            except SyntaxError:
    ...
    try:
        node = parse_expr(raw, lineno=lineno)
...
def parse_value(raw: str, lineno: int | None = None):
    ...
    if raw.startswith('[') and raw.endswith(']'):
        ...
        for item in re.split(r',\s*', items_str):
            items.append(parse_value(item.strip(), lineno=lineno))
        return items
    try:
        node = parse_expr(raw, lineno=lineno)
...
class Parser:
    def __init__(self, source: str, base_path: str = ""):
        self.lines, self.line_nos = tokenise(source)
        self.pos = 0
        ...
        while self.pos < len(self.lines):
            ...
            if m:
                ...
                try:
                    ...
                    # Объединяем хендлеры и сценарии
                    prog.handlers.extend(import_prog.handlers)
                    prog.scenarios.update(import_prog.scenarios)
            ...
            src_line = self.line_nos[self.pos]
            stmt = self._parse_stmt(self.consume(), src_line)
            if stmt:
    ...
    def _parse_block(self) -> list:
        ...
        while self.pos < len(self.lines):
            ...
            # переключить var: "значение": тело …
            m = re.match(r'^переключить\s+(\w+)\s*:\s*$', stripped)
            if m:
                self.consume()
                var_name = m.group(1)
                cases = []
                while self.pos < len(self.lines):
                    peek_line = self.peek()
                    peek_st = peek_line.strip()
                    if not peek_st:
                        self.consume()
                        continue
                    row_ind = indent_of(peek_line)
                    if row_ind <= base:
                        break
                    mc = re.match(r'^"([^"]*)"\s*:\s*$', peek_st)
                    if not mc:
                        break
                    self.consume()
                    cases.append((mc.group(1), self._parse_block()))
                if cases:
                    stmts.append(SwitchStmt(variable=var_name, cases=cases))
                continue
            ...
            if stripped.startswith("inline-кнопки:") or stripped == "inline-кнопки":
                dyn = re.match(
                    r'^inline-кнопки:\s*из\s+(\w+)\s+по\s+(\w+)\s*/\s*(\w+)(?:\s+callback=([^\s]+))?(?:\s+columns=(\d+))?(?:\s+append_back=(true|false))?\s*$',
                    stripped,
                    flags=re.IGNORECASE,
                )
                if dyn:
                    self.consume()
                    items_name = dyn.group(1)
                    text_field = dyn.group(2)
                    id_field = dyn.group(3)
                    callback_prefix = dyn.group(4) or ""
                    columns = int(dyn.group(5) or 1)
                    append_back = (dyn.group(6) or "true").lower() == "true"
                    stmts.append(
                        InlineKeyboardFromList(
                            items=Variable(items_name),
            ...
            if m:
                stmts.append(InlineKeyboardFromDB(
                    key=parse_value(m.group(1).strip(), lineno=self.line_nos[self.pos]),
            ...
            src_line = self.line_nos[self.pos]
            self.consume()
            stmt = self._parse_stmt(stripped, src_line)
            if stmt:
                stmts.append(stmt)

        return stmts

    def _parse_stmt(self, line: str, src_line: int):
        line = line.strip()
        pv = lambda r: parse_value(r, lineno=src_line)
        pse = lambda r: parse_string_expr(r, lineno=src_line)

        # ответ "текст" / ответ "текст" + переменная
        m = re.match(r'^ответ\s+(.+)$', line)
        if m:
            return Reply(pse(m.group(1)))

        # спросить "вопрос" → переменная  (и ASCII: ->)
        ...
        if m:
            return Remember(m.group(1), pv(m.group(2)))
        ...
        if m:
            return SendMarkdown(pse(m.group(1)))
        ...
        if m:
            k = m.group(1)
            _validate_quoted_template_fragment(k, 'ключ в «сохранить»', src_line)
            return SaveToDB(k, pv(m.group(2)))

        # получить "ключ" → переменная
        m = re.match(r'^получить\s+"([^"]+)"\s*→\s*(\w+)$', line)
        if m:
            k = m.group(1)
            _validate_quoted_template_fragment(k, 'ключ в «получить»', src_line)
            return LoadFromDB(m.group(2), k)
        ...
        m = re.match(r'^глобально\s+(\w+)\s*=\s*(.+)$', line)
        if m:
            return GlobalVar(m.group(1), pv(m.group(2)))
        ...
        if not m:
            m = re.match(r'^уведомить\s+(.+?)\s+"(.+)"$', line)
        if m:
            return Notify(user_id=pv(m.group(1).strip()),
                          parts=pse(f'"{m.group(2)}"'))

        # рассылка всем: "текст"
        m = re.match(r'^рассылка всем:\s*(.+)$', line)
        if m:
            return Broadcast(parts=pse(m.group(1)), segment="")

        # рассылка группе TAG: "текст"
        m = re.match(r'^рассылка группе\s+(\S+):\s*(.+)$', line)
        if m:
            return Broadcast(parts=pse(m.group(2)), segment=m.group(1))
        ...
        if m:
            p = m.group(1)
            _validate_quoted_template_fragment(p, 'путь в «json_файл»', src_line)
            return LoadJson(path=p, variable=m.group(2))
        m = re.match(r'^json_файл\s+(\w+)\s*→\s*(\w+)$', line)
        if m:
            return LoadJson(path=pv(m.group(1)), variable=m.group(2))

        # сохранить_json "путь" = переменная
        m = re.match(r'^сохранить_json\s+"([^"]+)"\s*=\s*(\w+)$', line)
        if m:
            p = m.group(1)
            _validate_quoted_template_fragment(p, 'путь в «сохранить_json»', src_line)
            return SaveJson(path=p, source_var=m.group(2))

        # удалить_файл "путь"
        m = re.match(r'^удалить_файл\s+"([^"]+)"$', line)
        if m:
            p = m.group(1)
            _validate_quoted_template_fragment(p, 'путь в «удалить_файл»', src_line)
            return DeleteFile(path=p)
        m = re.match(r'^удалить_файл\s+(\w+)$', line)
        if m:
            return DeleteFile(path=pv(m.group(1)))
        ...
        if m:
            return DeleteDictKey(target=m.group(1), key=pv(m.group(2)))

        # объект["ключ"] = значение — присваивание поля dict
        m = re.match(r'^(\w+)\["([^"]+)"\]\s*=\s*(.+)$', line)
        if m:
            return SetDictKey(target=m.group(1), key=m.group(2), value=pv(m.group(3)))
        m = re.match(r'^(\w+)\[(\w+)\]\s*=\s*(.+)$', line)
        if m:
            return SetDictKey(target=m.group(1), key=pv(m.group(2)),
                              value=pv(m.group(3)))
        ...
        if m:
            return HttpPatch(url=m.group(1), data=pv(m.group(2)), variable=m.group(3))
        ...
        if m:
            return HttpPut(url=m.group(1), data=pv(m.group(2)), variable=m.group(3))
        ...
        if m:
            return HttpPost(url=m.group(1), data=pv(m.group(2)), variable=m.group(3))
        ...
        if m:
            k = m.group(1)
            _validate_quoted_template_fragment(k, 'ключ в «удалить»', src_line)
            return DeleteFromDB(key=k)
        ...
        if m:
            k = m.group(1)
            _validate_quoted_template_fragment(k, 'ключ в «сохранить_глобально»', src_line)
            return SaveGlobalDB(key=k, value=pv(m.group(2)))

        # получить от USER_ID "ключ" → переменная
        m = re.match(r'^получить от\s+(.+?)\s+"([^"]+)"\s*→\s*(\w+)$', line)
        if m:
            k = m.group(2)
            _validate_quoted_template_fragment(k, 'ключ в «получить от»', src_line)
            return LoadFromUserDB(user_id=pv(m.group(1).strip()),
                                  key=k, variable=m.group(3))
        ...
        if m:
            return ReturnValue(value=pv(m.group(1)))

        _raise_syntax(f"Неизвестная или неподдерживаемая инструкция Cicada: {line!r}", src_line)
