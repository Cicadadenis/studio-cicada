...
const INNER_RULES = [
  ...
  { key: 'переслать фото', re: /^переслать\s+фото/i, snippet: 'переслать фото' },
  { key: 'inline из бд', re: /^inline-кнопки\s+из\s+бд\s+.+/i, snippet: 'inline-кнопки из бд "категории" текст "name" callback "category:" назад "⬅️ Назад" → "назад"' },
  ...
  { key: 'переслать', re: /^переслать(?!\s+фото\b)(?:\s+сообщение)?\s+.+/i, snippet: 'переслать user' },
